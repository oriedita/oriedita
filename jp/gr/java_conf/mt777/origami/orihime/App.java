...
public class App extends Frame implements ActionListener, MouseListener, MouseMotionListener, MouseWheelListener {
    ...
    int i_SubThread = 0;//SubThread（折畳み計算）実行中なら１、実行していないなら０
    ...
    boolean w_image_jikkoutyuu = false;//折畳みまとめ実行の。単一回のイメージ書き出しが実行中ならtureになる。
    //boolean matome_write_image_jikkoutyuu = false;//matome_write_imageが実行中ならtureになる。これは、複数の折りあがり形の予測の書き出しがかすれないように使う。20170613

    String fname_and_number;//まとめ書き出しに使う。


    public int i_anti_alias = 0;//展開図のアンチェイリアスをするかしないか。する=1、しない=0
    public double senhaba_for_anti_alias = 1.0;//展開図のアンチェイリアスをするなら=1.2、しない=1.0

    public int ir_ten = 1;//展開図の点の形状を指定

    public int i_orisen_hyougen = 1;//折線の表現、1＝色、2=色と形状、3=黒で1点鎖線、4=黒で2点鎖線
    ...
    //JDialog dialog = new JDialog(this , "こんにちは" , true);//ダイアログのタイトル

    //int step;                             //操作の指定

    int i_orisen_hojyosen = 0;//=0は折線入力　=1は補助線入力モード
    ...
    //Label lbel0 ;                         //対称性の指定に用いる
    //Label lbel1 ;                         //対称性の指定に用いる
    //Button	Button0 ;
    //Button	Button0a ;

    int kakudokei_input_id = 1;//角度系の入力方法の指定　kakudokei_input_id=1は線分指定、2は2点指定
    ...
    //int i_fold_type=0;//=0なにもしない、=1通常の展開図の全折線を対象とした折り畳み推定、=2はselectされた折線を対象とした折り畳み推定、=3は折畳み状態を変更

    JButton Button0b;                    //対称性の指定に用いる
    //Button Button1   ;                    //操作の指定に用いる
    ...
    //Button ButtonCol_gray   ;                    //折線の色の指定に用いる
    ...
    JButton Button_haikei_Lock_on;//背景のロックオン
    JButton Button_haikei_kirikae;//背景を表示するかどうかの指定

    //Button ButtonCol_green15   ;                    //折線の色の指定に用いる
    //Button ButtonCol_green20  ;                    //折線の色の指定に用いる
    //Button ButtonCol_green22p5   ;                    //折線の色の指定に用いる
    ...
    JTextField text2;
    int i_orisen_bunkatu_suu = 1;//free折線入力で、折線の等分割されている数
    ...
    JTextField text26;
    int i_oritatami_bangou = 1;//折り畳み推定の何番目を表示するか指定
    ...
    public JCheckBox ckbox_mejirusi;//十字や基準面などの目印画線
    ...
    int ihaikeihyouji = 0;//0なら背景を表示しない。1なら表示する。2は無い。
    int ikaisetuhyouji = 1;//0なら解説を表示しない。1なら表示する。2は無い。
    public int iTenkaizuSenhaba = 1;//展開図の線の太さ。
    public int i_h_TenkaizuSenhaba = 3;//非干渉補助線の線の太さ
    ...
    public int i_mouse_modeA = 71;//マウスの動作に対する反応を規定する。1なら線分入力モード。2なら展開図調整(移動)。101なら折り上がり図の操作。
    ...
    int i_sub_mode = 0;
    //i_sub_mode サブスレッドの動作規定。
    //0=折畳推定5の実行。折畳み推定の別解をまとめて出すモードではない。
    //1=折畳推定5の実行。折畳み推定の別解をまとめて出す。
    //2=
    ...
    int flg61 = 0;//枠設定時につかう　20180524

    int flg_wi = 0;//writeimage時につかう　1にするとpaintの関数の終了部にwriteimageするようにする。これは、paintの変更が書き出されるイメージに反映されないことを防ぐための工夫。20180528
    String fname_wi;


    public int i_sel_mou_mode;//セレクトしてマウス操作をする際にどの操作をするかの指定。セレクト後にセレクト済みの点を選択し自動的にセレクト前提のマウス操作に切り替えるときに使う。
    ...
    Frame add_frame;
    int i_add_frame = 0;//1=add_frameが存在する。,0=存在しない。

    //int ckbox_add_frame_to_front_isSelected=1;//1=add_frameは常に前面。,0前後は、操作に応じて。 20200930
    int ckbox_add_frame_SelectAnd3click_isSelected = 0;//1=折線セレクト状態でトリプルクリックするとmoveやcopy等の動作モードに移行する。 20200930
...
// **************************************************************************************************************
    ...
    public App() {
        ...
        show();                 //アプレットの時は使わない。アプリケーションの時は使う。かな
////b* アプリケーション用。先頭が／＊／／／で始まる行にはさまれた部分は無視される。
...
//	ArrayList OAZ = new ArrayList(); //折り上がり図のインスタンス化
        ...
        camera_of_orisen_nyuuryokuzu.setDisplayPositionY(350.0);
...
// -----------------20180503追加
                    d_syukusyaku_keisuu = camera_of_orisen_nyuuryokuzu.getCameraZoomX();
                    text27.setText(String.valueOf(d_syukusyaku_keisuu)); //縮尺係数
                    text27.setCaretPosition(0);

                    d_kaiten_hosei = camera_of_orisen_nyuuryokuzu.getCameraAngle();
...
// -----61;長方形内選択モード。
        ...
        Button_tyouhoukei_select.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                readImageFromFile3();
                i_orisen_hojyosen = 0;//=0は折線入力　=1は補助線入力モード
                es1.set_i_orisen_hojyosen(i_orisen_hojyosen);//このボタンと機能は補助絵線共通に使っているのでi_orisen_hojyosenの指定がいる
                i_mouse_modeA = 61;
...
//点探し
        ...
        ckbox_point_search.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
...
//		JButton	Button_tenkaizu_syukusyou	= new JButton(	"CP_z_out"	);Button_tenkaizu_syukusyou.addActionListener(new ActionListener(){public void actionPerformed(ActionEvent e) {
                ...
                camera_of_orisen_nyuuryokuzu.kakezan_camera_zoom_x(d_bairitu);
                camera_of_orisen_nyuuryokuzu.kakezan_camera_zoom_y(d_bairitu);
...
//20180122追加
                ...
                for (int i_oz = 1; i_oz <= OAZ.size() - 1; i_oz++) {
                    ...
                    OZi.camera_of_oriagari_front.kakezan_camera_zoom_x(d_bairitu);
                    OZi.camera_of_oriagari_front.kakezan_camera_zoom_y(d_bairitu);

                    OZi.camera_of_oriagari_rear.camera_ichi_sitei_from_TV(t_o2tv);
                    OZi.camera_of_oriagari_rear.kakezan_camera_zoom_x(d_bairitu);
                    OZi.camera_of_oriagari_rear.kakezan_camera_zoom_y(d_bairitu);
                    ...
                    OZi.camera_of_transparant_rear.kakezan_camera_zoom_x(d_bairitu);
                    OZi.camera_of_transparant_rear.kakezan_camera_zoom_y(d_bairitu);
        ...
        Button_syukusyaku_keisuu_set.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                if (d_syukusyaku_keisuu != d_syukusyaku_keisuu_old) {
                    ...
                    camera_of_orisen_nyuuryokuzu.setCameraZoomX(d_syukusyaku_keisuu);
                    camera_of_orisen_nyuuryokuzu.setCameraZoomY(d_syukusyaku_keisuu);
...
// *****北*************展開の回転************************************************************
        ...
        Button_tenkaizu_p_kaiten.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                d_kaiten_hosei = oc.angle_between_m180_180(d_kaiten_hosei + 11.25);
                camera_of_orisen_nyuuryokuzu.setCameraAngle(d_kaiten_hosei);
...
// -----回転角度補正set
        ...
        Button_kaiten_hosei_set.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                d_kaiten_hosei = oc.angle_between_m180_180(String2double(text28.getText(), d_kaiten_hosei_old));
...
//offsc_haikei  = new BufferedImage(2000, 1100,  BufferedImage.TYPE_INT_BGR)	;//TYPE_INT_BGRだとトリムした画像を回転したりすると色が落ちる20181206
                ...
                if ((img_haikei != null) && (ihaikeihyouji >= 1)) {
                    int iw = img_haikei.getWidth(null);//イメージの幅を取得
                    int ih = img_haikei.getHeight(null);//イメージの高さを取得
...
//枠設定時の背景を枠内のみ残してトリム 20181204
                if ((i_mouse_modeA == 61) && (es1.get_i_egaki_dankai() == 4)) {//枠線が表示されている状態
...
// -------------71;折り畳み可能線+格子点系入力
        ...
        Button_oritatami_kanousen_and_kousitenkei_simple.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                img_kaisetu_fname = "qqq/oritatami_kanousen_and_kousitenkei_simple.png";
                readImageFromFile3();

                i_mouse_modeA = 71;
                iro_sitei_ato_ni_jissisuru_sagyou_bangou = 71;
                System.out.println("i_mouse_modeA = " + i_mouse_modeA);
...
// -----1;線分分割数set
        ...
        Button_senbun_bunkatu_set.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                i_orisen_bunkatu_suu = StringOp.String2int(text2.getText(), i_orisen_bunkatu_suu_old);
...
// -------------;黒線のみの線分削除モード。
        ...
        Button_kuro_lineSegment_removal.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                i_orisen_hojyosen = 2;//=2は黒の折線削除モード
                es1.set_i_orisen_hojyosen(i_orisen_hojyosen);
...
// -------------;線分削除モード。消しゴム(補助活線のみ)
        ...
        Button_senbun3_sakujyo.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                i_orisen_hojyosen = 3;//=0は折線入力　=1は補助線入力モード =3は補助活線のみ対象
                es1.set_i_orisen_hojyosen(i_orisen_hojyosen);
...
// -----1;	角数set
        ...
        Button_kakusuu_set.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                i_sei_takakukei = StringOp.String2int(text9.getText(), i_sei_takakukei_old);
...
// -------------50;2円を指定し、それぞれの円に同心円を加える。それぞれの同心円の組にできる帯領域の幅が等しくなるようにして、加えられた同心円同士が接するようにする。
...
// -------------h_1;補助線入力モード。
        ...
        Button_h_senbun_nyuryoku.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                Button_kyoutuu_sagyou();
                repaint();
                i_orisen_hojyosen = 1;//=0は折線入力　=1は補助線入力モード
                es1.set_i_orisen_hojyosen(i_orisen_hojyosen);
            }
        });
...
// -------------追加フレーム表示
        ...
        Button_tuika_kinou.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                img_kaisetu_fname = "qqq/tuika_kinou.png";
                readImageFromFile3();
                //i_mouse_modeA=57;
                //System.out.println("i_mouse_modeA = "+i_mouse_modeA);
...
// *******南***************************************************** //System.out.println("裏表");*************
        ...
        Button0b.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                readImageFromFile3();
                //ip4==0 表、ip4==1　裏、ip4==2、ip4==3　両方と透過
                OZ.ip4 = OZ.ip4 + 1;
                if (OZ.ip4 == 4) {
                    OZ.ip4 = 0;
                }
                if ((i_mouse_modeA == 101) && (OZ.ip4 == 2)) {
                    OZ.ip4 = 0;
                }//折り上がり予想図動かしのとき、動かしできなくなるモードを避けるために追加
                Button_kyoutuu_sagyou();
                repaint();
            }
        });
...
// -----;	//折り畳み推定の指定番目を表示する
        ...
        Button_bangou_sitei_suitei_hyouji.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                i_oritatami_bangou = StringOp.String2int(text26.getText(), i_oritatami_bangou_old);
...
// ********南*****************************************************************
        ...
        Button_oriagari_syukusyou.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                OZ.camera_of_foldedFigure.setCameraZoomX(OZ.d_foldedFigure_syukusyaku_keisuu);
                OZ.camera_of_foldedFigure.setCameraZoomY(OZ.d_foldedFigure_syukusyaku_keisuu);

                OZ.camera_of_oriagari_front.setCameraZoomX(OZ.d_foldedFigure_syukusyaku_keisuu);
                OZ.camera_of_oriagari_front.setCameraZoomY(OZ.d_foldedFigure_syukusyaku_keisuu);

                OZ.camera_of_oriagari_rear.setCameraZoomX(OZ.d_foldedFigure_syukusyaku_keisuu);
                OZ.camera_of_oriagari_rear.setCameraZoomY(OZ.d_foldedFigure_syukusyaku_keisuu);

                OZ.camera_of_transparent_front.setCameraZoomX(OZ.d_foldedFigure_syukusyaku_keisuu);
                OZ.camera_of_transparent_front.setCameraZoomY(OZ.d_foldedFigure_syukusyaku_keisuu);

                OZ.camera_of_transparant_rear.setCameraZoomX(OZ.d_foldedFigure_syukusyaku_keisuu);
                OZ.camera_of_transparant_rear.setCameraZoomY(OZ.d_foldedFigure_syukusyaku_keisuu);
...
// *****南********************************************************************
        ...
        Button_oriagari_p_kaiten.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                OZ.d_foldedFigure_kaiten_hosei = oc.angle_between_m180_180(OZ.d_foldedFigure_kaiten_hosei + 11.25);
                OZ.camera_of_foldedFigure.setCameraAngle(OZ.d_foldedFigure_kaiten_hosei);
                OZ.camera_of_oriagari_front.setCameraAngle(OZ.d_foldedFigure_kaiten_hosei);
                OZ.camera_of_oriagari_rear.setCameraAngle(OZ.d_foldedFigure_kaiten_hosei);
                OZ.camera_of_transparent_front.setCameraAngle(OZ.d_foldedFigure_kaiten_hosei);
                OZ.camera_of_transparant_rear.setCameraAngle(OZ.d_foldedFigure_kaiten_hosei);
...
// -----回転角度補正set
        ...
        Button_oriagarizu_kaiten_hosei_set.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                OZ.d_foldedFigure_kaiten_hosei = oc.angle_between_m180_180(String2double(text30.getText(), d_oriagarizu_kaiten_hosei_old));
...
//bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb
            ...
            if (i_SubThread == 0) {
                i_SubThread = 1;
                i_sub_mode = 0;//1=折畳み推定の別解をまとめて出す。0=折畳み推定の別解をまとめて出すモードではない。この変数はサブスレッドの動作変更につかうだけ。20170611にVer3.008から追加
                mks();//新しいスレッドを作る
                sub.start();
            }
...
//北辺
        ...
        es1.set_i_kou_mitudo_nyuuryoku(0);          //高密度入力するかどうかの選択
        ...
        ckbox_mejirusi.setSelected(true);//十字や基準面などの目印画線
...
//西辺
        ...
        text2.setText(String.valueOf(i_orisen_bunkatu_suu));
        es1.set_i_orisen_bunkatu_suu(i_orisen_bunkatu_suu);//フリー折線入力時の分割数
...
// ---------------------------------------
    ...
    int i_cp_or_oriagari = 0;//マウスホイールの対象がcp展開図なら0、折り上がり図(表)なら1、折り上がり図(裏)なら2、透過図(表)なら3、透過図(裏)なら4
...
//for (int i=1;i<=OAZ.size()-1;i++){set_i_OAZ(i);}
    ...
    public Point e2p(MouseEvent e) {
        ...
        if (ckbox_ten_hanasi.isSelected()) {
            d_haba = camera_of_orisen_nyuuryokuzu.getCameraZoomX() * es1.get_d_hantei_haba();
        }
...
//10001;test1 入力準備として点を３つ指定する
    ...
    public void mouseMoved(MouseEvent e) {
        ...
        else if (i_mouse_modeA == 63) {
            es1.setCamera(camera_of_orisen_nyuuryokuzu);
            es1.mMoved_A_63(p);
        }//外周部折り畳みチェックに使う
        else if (i_mouse_modeA == 64) {
            es1.setCamera(camera_of_orisen_nyuuryokuzu);
            es1.mMoved_A_64(p);
        }//線内削除　に使う
        else if (i_mouse_modeA == 65) {
            es1.setCamera(camera_of_orisen_nyuuryokuzu);
            es1.mMoved_A_65(p);
        }//lX　直線で折線削除に使う
        else if (i_mouse_modeA == 66) {
            es1.setCamera(camera_of_orisen_nyuuryokuzu);
            es1.mMoved_A_66(p);
        }//選択＿多角形　に使う
        else if (i_mouse_modeA == 67) {
            es1.setCamera(camera_of_orisen_nyuuryokuzu);
            es1.mMoved_A_67(p);
        }//非選択＿多角形　に使う
        else if (i_mouse_modeA == 68) {
            es1.setCamera(camera_of_orisen_nyuuryokuzu);
            es1.mMoved_A_68(p);
        }//選択＿ｌX　に使う
        else if (i_mouse_modeA == 69) {
            es1.setCamera(camera_of_orisen_nyuuryokuzu);
            es1.mMoved_A_69(p);
        }//非選択＿ｌX　　に使う
        else if (i_mouse_modeA == 70) {
            es1.setCamera(camera_of_orisen_nyuuryokuzu);
            es1.mMoved_A_70(p);
        }//線分延長モード(延長する元の折線のクリックだけで実行されるタイプ)　に使う
        else if (i_mouse_modeA == 71) {
            es1.setCamera(camera_of_orisen_nyuuryokuzu);
            es1.mMoved_A_71(p);
        }//複数の線分延長モード　に使う
...
//wwwwwwwwwwww
        ...
        if (btn == MouseEvent.BUTTON1) {
            ...
            if (i_mouse_modeA == 62) {//ボロノイ図入力時は、入力途中のボロノイ母点が消えないように、右クリックに反応させない。20181208
            } else {
                ...
                i_orisen_hojyosen = 4;//=0は折線入力　=1は補助線入力モード、４は両方に作用
                es1.set_i_orisen_hojyosen(i_orisen_hojyosen);
        ...
        //else if(i_mouse_modeA==72) { es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mPressed_A_72(p);}//unselect　に使う
        //else if(i_mouse_modeA==73) { es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mPressed_A_73(p);}//unselect　に使う
        //else if(i_mouse_modeA==74) { es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mPressed_A_74(p);}//unselect　に使う
        //else if(i_mouse_modeA==75) { es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mPressed_A_75(p);}//unselect　に使う
        //else if(i_mouse_modeA==76) { es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mPressed_A_76(p);}//unselect　に使う
        //else if(i_mouse_modeA==77) { es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mPressed_A_77(p);}//unselect　に使う
        //else if(i_mouse_modeA==78) { es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mPressed_A_78(p);}//unselect　に使う
        //else if(i_mouse_modeA==79) { es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mPressed_A_79(p);}//unselect　に使う
        //else if(i_mouse_modeA==80) { es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mPressed_A_80(p);}//unselect　に使う
        //else if(i_mouse_modeA==81) { es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mPressed_A_81(p);}//unselect　に使う
        //else if(i_mouse_modeA==82) { es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mPressed_A_82(p);}//unselect　に使う
        //else if(i_mouse_modeA==83) { es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mPressed_A_83(p);}//unselect　に使う
        //else if(i_mouse_modeA==84) { es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mPressed_A_84(p);}//unselect　に使う
        //else if(i_mouse_modeA==85) { es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mPressed_A_85(p);}//unselect　に使う
        //else if(i_mouse_modeA==86) { es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mPressed_A_86(p);}//unselect　に使う
        //else if(i_mouse_modeA==87) { es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mPressed_A_87(p);}//unselect　に使う
        //else if(i_mouse_modeA==88) { es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mPressed_A_88(p);}//unselect　に使う
        //else if(i_mouse_modeA==89) { es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mPressed_A_89(p);}//unselect　に使う
    ...
    public void mouseDragged(MouseEvent e) {
        ...
        if (i_mouseDragged_yuukou == 1) {
            ...
            if (btn == MouseEvent.BUTTON1) {
                ...
                if (i_cp_or_oriagari == 0) {// 展開図移動。
                    camera_of_orisen_nyuuryokuzu.hyouji_ichi_idou(mouse_temp0.other_Point_position(p));
                    es1.setCamera(camera_of_orisen_nyuuryokuzu);
                } else if (i_cp_or_oriagari == 1) {
                    OZ.camera_of_oriagari_front.hyouji_ichi_idou(mouse_temp0.other_Point_position(p));
                } else if (i_cp_or_oriagari == 2) {
                    OZ.camera_of_oriagari_rear.hyouji_ichi_idou(mouse_temp0.other_Point_position(p));
                } else if (i_cp_or_oriagari == 3) {
                    OZ.camera_of_transparent_front.hyouji_ichi_idou(mouse_temp0.other_Point_position(p));
                } else if (i_cp_or_oriagari == 4) {
                    OZ.camera_of_transparant_rear.hyouji_ichi_idou(mouse_temp0.other_Point_position(p));
                }
...
//OZi.d_oriagarizu_syukusyaku_keisuu=OZi.d_oriagarizu_syukusyaku_keisuu*d_bairitu;
                    ...
                    OZi.camera_of_foldedFigure.hyouji_ichi_idou(mouse_temp0.other_Point_position(p));
                    //OZi.camera_of_oriagarizu.kakezan_camera_bairitsu_x(d_bairitu);
                    //OZi.camera_of_oriagarizu.kakezan_camera_bairitsu_y(d_bairitu);

                    OZi.camera_of_oriagari_front.hyouji_ichi_idou(mouse_temp0.other_Point_position(p));
                    ...
                    OZi.camera_of_oriagari_rear.hyouji_ichi_idou(mouse_temp0.other_Point_position(p));
                    ...
                    OZi.camera_of_transparent_front.hyouji_ichi_idou(mouse_temp0.other_Point_position(p));
                    ...
                    OZi.camera_of_transparant_rear.hyouji_ichi_idou(mouse_temp0.other_Point_position(p));
...
//20180225追加　ここまで
            ...
            //else if(i_mouse_modeA==72) {es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mDragged_A_72(p);}
            //else if(i_mouse_modeA==73) {es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mDragged_A_73(p);}
            //else if(i_mouse_modeA==74) {es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mDragged_A_74(p);}
            //else if(i_mouse_modeA==75) {es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mDragged_A_75(p);}
            //else if(i_mouse_modeA==76) {es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mDragged_A_76(p);}
            //else if(i_mouse_modeA==77) {es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mDragged_A_77(p);}
            //else if(i_mouse_modeA==78) {es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mDragged_A_78(p);}
            //else if(i_mouse_modeA==79) {es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mDragged_A_79(p);}
            //else if(i_mouse_modeA==80) {es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mDragged_A_80(p);}
            //else if(i_mouse_modeA==81) {es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mDragged_A_81(p);}
            //else if(i_mouse_modeA==82) {es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mDragged_A_82(p);}
            //else if(i_mouse_modeA==83) {es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mDragged_A_83(p);}
            //else if(i_mouse_modeA==84) {es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mDragged_A_84(p);}
            //else if(i_mouse_modeA==85) {es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mDragged_A_85(p);}
            //else if(i_mouse_modeA==86) {es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mDragged_A_86(p);}
            //else if(i_mouse_modeA==87) {es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mDragged_A_87(p);}
            //else if(i_mouse_modeA==88) {es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mDragged_A_88(p);}
            //else if(i_mouse_modeA==89) {es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mDragged_A_89(p);}
...
//20180225追加　ここまで
            ...
            //else if(i_mouse_modeA==72){es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mReleased_A_72(p);}
            //else if(i_mouse_modeA==73){es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mReleased_A_73(p);}
            //else if(i_mouse_modeA==74){es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mReleased_A_74(p);}
            //else if(i_mouse_modeA==75){es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mReleased_A_75(p);}
            //else if(i_mouse_modeA==76){es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mReleased_A_76(p);}
            //else if(i_mouse_modeA==77){es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mReleased_A_77(p);}
            //else if(i_mouse_modeA==78){es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mReleased_A_78(p);}
            //else if(i_mouse_modeA==79){es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mReleased_A_79(p);}
            //else if(i_mouse_modeA==80){es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mReleased_A_80(p);}
            //else if(i_mouse_modeA==81){es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mReleased_A_81(p);}
            //else if(i_mouse_modeA==82){es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mReleased_A_82(p);}
            //else if(i_mouse_modeA==83){es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mReleased_A_83(p);}
            //else if(i_mouse_modeA==84){es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mReleased_A_84(p);}
            //else if(i_mouse_modeA==85){es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mReleased_A_85(p);}
            //else if(i_mouse_modeA==86){es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mReleased_A_86(p);}
            //else if(i_mouse_modeA==87){es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mReleased_A_87(p);}
            //else if(i_mouse_modeA==88){es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mReleased_A_88(p);}
            //else if(i_mouse_modeA==89){es1.setCamera(camera_of_orisen_nyuuryokuzu);es1.mReleased_A_89(p);}
...
*/
    ...
    public void paint(Graphics g) {
        ...
        if (OZ.different_search_flg == 1) {
            ...
            Button_bangou_sitei_suitei_hyouji.setBackground(new Color(200, 200, 200));//これがないとForegroundが直ぐに反映されない。仕様なのか？
...
img_haikei=(Image)imageT;
			int iw=img_haikei.getWidth(this);//イメージの幅を取得
			int ih=img_haikei.getHeight(this);//イメージの高さを取得
...
*/
        ...
        //格子表示
        //es1.kousi_oekaki_with_camera(bufferGraphics,i_bun_hyouji,i_cp_hyouji,i_a0_hyouji,i_a1_hyouji,fTenkaizuSenhaba,i_orisen_hyougen,f_h_TenkaizuSenhaba,dim.width,dim.height);//渡す情報はカメラ設定、線幅、画面X幅、画面y高さ


        //解説表示
        //System.out.println("paint　+++++++++++++++++++++　解説表示  " +ikaisetuhyouji );
        if ((img_kaisetu != null) && (ikaisetuhyouji >= 1)) {
            ...
            //	System.out.println("paint幅＝"+img_haikei.getWidth(this));
            //	System.out.println("paint高さ＝"+img_haikei.getHeight(this));
        }
        ...
        double d_haba = camera_of_orisen_nyuuryokuzu.getCameraZoomX() * es1.get_d_hantei_haba();
        //懐中電灯（点の）探索範囲
        ...
        //System.out.println("paint　+++++++++++++++++++++　paint終了");
...
*/
        ...
        if (OZ.matome_write_image_jikkoutyuu) {//まとめ書き出し実行中の意味)
        ...
        if (flg_wi == 1) {//枠あり書き出し時の制御用20180525
            flg_wi = 0;
...
/*        void suitei_syokika(){
		...
		OZ.hyouji_flg=0;//折り上がり図の表示様式の指定。1なら展開図整理、2なら針金図。3なら透過図。5なら実際に折り紙を折った場合と同じ。
		OZ.i_suitei_meirei=0;//折り畳み推定をどの段階まで行うかの指示
		OZ.i_suitei_dankai=0;//折り畳み推定がどの段階までできたかの表示
		OZ.betu_sagasi_flg=0;

		OZ.matome_write_image_jikkoutyuu = false; //複数の折りあがり形の予測の書き出しがが実行中ならtureになる。20170615
...
//repaint();
        //System.out.println("01幅＝"+img_haikei.getWidth(this));
        //System.out.println("01高さ＝"+img_haikei.getHeight(this));
    ...
    void writeImageFile(String fname) {//i=1　png, 2=jpg
        ...
        if (fname != null) {
            ...
            try {
                if (flg61 == 1) { //枠設定時の枠内のみ書き出し 20180524
                    int xmin = (int) es1.p61_TV_hako.get_x_min();
                    int xmax = (int) es1.p61_TV_hako.get_x_max();
                    int ymin = (int) es1.p61_TV_hako.get_y_min();
                    int ymax = (int) es1.p61_TV_hako.get_y_max();
                    //	es1.set_i_egaki_dankai(0);repaint();//緑の枠線を書き出さないために必要
                    //try{
                    //	Thread.sleep(1000);
                    //}catch (InterruptedException e){
                    //}
...
// ------------------------------------------
    ...
    void readImageFromFile2() {//これは使っていないので消していい
        ...
        repaint();
        System.out.println("02幅＝" + img_haikei.getWidth(this));
        System.out.println("02高さ＝" + img_haikei.getHeight(this));
...
////b* アプリケーション用。先頭が／＊／／／で始まる行にはさまれた部分は無視される
    ...
    //--------------------------------------------------------------------------
    void oritatami_suitei_2col() {//２色塗りわけ展開図
    ...
    public void check4(double r) {
        ...
        //es1.ap_check4(r);
        if (i_SubThread == 0) {
            i_sub_mode = 3;//3=頂点周りの折畳み可能性判定、1=折畳み推定の別解をまとめて出す。0=折畳み推定の別解をまとめて出すモードではない。この変数はサブスレッドの動作変更につかうだけ。20170611にVer3.008から追加
