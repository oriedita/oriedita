...
public class Egaki_Syokunin {
    ...
    Ten moyori_ten = new Ten(100000.0, 100000.0); //マウス最寄の点。get_moyori_ten(Ten p)で求める。
    Senbun moyori_senbun = new Senbun(100000.0, 100000.0, 100000.0, 100000.1); //マウス最寄の線分
    Senbun moyori_step_senbun = new Senbun(100000.0, 100000.0, 100000.0, 100000.1); //マウス最寄のstep線分(線分追加のための準備をするための線分)。なお、ここで宣言する必要はないので、どこで宣言すべきか要検討20161113
    ...
    public Orisensyuugou ori_s = new Orisensyuugou();    //折線を格納する
    Orisensyuugou hoj_s = new Orisensyuugou();    //補助線を格納する


    public Orisensyuugou ori_v = new Orisensyuugou();    //ボロノイ図の線を格納する

    Egaki_Syokunin_Dougubako e_s_dougubako = new Egaki_Syokunin_Dougubako(ori_s);

    private final Senbunsyuugou sen_s = new Senbunsyuugou();    //基本枝構造のインスタンス化
    ...
    Senbun[] s_step = new Senbun[1024];//描画時の一時表示用に使う。s_step[0] は実際は使わず、s_step[1]から使う。
    En[] e_step = new En[1024];//描画時の一時表示用に使う。e_step[0] は実際は使わず、e_step[1]から使う。
    ...
    En[] e_kouho = new En[16];//描画時の選択候補表示用に使う。e_kouho[0] は実際は使わず、e_kouho[1]から使う。
    ...
    Ten p61_1 = new Ten();//TV座標
    Ten p61_2 = new Ten();//TV座標
    Ten p61_3 = new Ten();//TV座標
    Ten p61_4 = new Ten();//TV座標
...
*/
    ...
    public void setMemo_for_yomikomi_tuika(Memo memo1) {//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<データ読み込み用
        ...
        Orisensyuugou ori_s_temp = new Orisensyuugou();    //追加された折線だけ取り出すために使う
    ...
    public Senbunsyuugou get_for_select_oritatami() {//selectした折線で折り畳み推定をする。
    ...
    //------------------------
    public Memo getMemo_for_kakidasi() {//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<データ書き出し
    ...
    //------------------------svgデータ書き出し
    public Memo getMemo_for_svg_kakidasi_with_camera(int i_bun_hyouji, int i_cp_hyouji, int i_a0_hyouji, int i_a1_hyouji, float fTenkaizuSenhaba, int i_orisen_hyougen, float f_h_TenkaizuSenhaba, int p0x_max, int p0y_max, int i_mejirusi_hyouji) {//引数はカメラ設定、線幅、画面X幅、画面y高さ
        Memo memo_temp = new Memo();
...
//--------------------------
    ...
    //--------------------------------------------
    public int addsenbun(Senbun s0) {//0=変更なし、1=色の変化のみ、2=線分追加
...
//	Ten p =new Ten();
    ...
    public void mMoved_m_001(Ten p0, int i_c) {//マウスで選択できる候補点を表示する。近くに既成の点があるときはその点が候補点となる。近くに既成の点が無いときは候補点無しなので候補点の表示も無し。
        if (i_kou_mitudo_nyuuryoku == 1) {
    ...
    //動作モデル003--------------------------------------------------------------------------------------------------------
    //マウス操作(マウスを動かしたとき)を行う関数
    public void mMoved_m_003(Ten p0, int i_c) {//マウスで選択できる候補点を表示する。常にマウスの位置自身が候補点となる。
        if (i_kou_mitudo_nyuuryoku == 1) {
            //s_kouho[1].setiactive(3);
            p.set(camera.TV2object(p0));
            i_kouho_dankai = 1;
    ...
    //マウスドラッグ---------------------------------------------------
    public void mDragged_m_00b(Ten p0, int i_c) {  //近くの既成点かマウス位置表示
...
*/
        ...
        if (i_mouse_modeA_62_ten_kasanari == 0) {
            ...
            Orisensyuugou ori_v_temp = new Orisensyuugou();    //修正用のボロノイ図の線を格納する
    ...
    //--------------------------------------------
    public int addsenbun_voronoi(Senbun s0) {//0=変更なし、1=色の変化のみ、2=線分追加
        ...
        for (int i_e_d = 1; i_e_d <= i_egaki_dankai; i_e_d++) {
            if (i_e_d != tyuusinn_ten_bangou) {
                ...
                if (i_e_d < tyuusinn_ten_bangou) {
                    ...
                    add_senbun.setcolor(tyuusinn_ten_bangou);//ボロノイ線分の2つのボロノイ頂点はiactiveとcolorに記録する
                } else {
                    add_senbun.setiactive(tyuusinn_ten_bangou);
                    add_senbun.setcolor(i_e_d);//ボロノイ線分の2つのボロノイ頂点はiactiveとcolorに記録する
                }
...
//------
    ...
    //マウス操作(マウスを動かしたとき)を行う関数    //System.out.println("_");
    public void mMoved_A_70(Ten p0) {
        mMoved_A_05or70(p0);
    }//常にマウスの位置のみが候補点

    //マウス操作(ボタンを押したとき)時の作業
    public void mPressed_A_70(Ten p0) {
        mPressed_A_05or70(p0);
    }

    //マウス操作(ドラッグしたとき)を行う関数
    public void mDragged_A_70(Ten p0) {
        mDragged_A_05or70(p0);
    }

    //マウス操作(ボタンを離したとき)を行う関数
    public void mReleased_A_70(Ten p0) {
        mReleased_A_05or70(p0);
    }
...
//-------------------------------------------------------------------------------------------------------------------------------
    ...
    //マウス操作(マウスを動かしたとき)を行う関数    //System.out.println("_");
    public void mMoved_A_05or70(Ten p0) {
        mMoved_m_003(p0, icol);
    }//常にマウスの位置のみが候補点

    //マウス操作(ボタンを押したとき)時の作業
    public void mPressed_A_05or70(Ten p0) {
        p.set(camera.TV2object(p0));
        i_kouho_dankai = 0;
    ...
    //マウス操作(ドラッグしたとき)を行う関数
    public void mDragged_A_05or70(Ten p0) {
        p.set(camera.TV2object(p0));
        if (i_egaki_dankai == 1) {
    ...
    //マウス操作(ボタンを離したとき)を行う関数
    public void mReleased_A_05or70(Ten p0) {
        p.set(camera.TV2object(p0));
        ...
        if (i_egaki_dankai == 1) {
            ...
            if ((entyou_kouho_nbox.getsousuu() == 0) && (s_step[1].getnagasa() <= 0.000001)) {//延長する候補になる折線を選ぶために描いた線分s_step[1]が点状のときの処理
                if (oc.kyori_senbun(p, moyori_senbun) < d_hantei_haba) {
                    ...
                    s_step[1].setb(oc.sentaisyou_ten_motome(moyori_senbun.geta(), moyori_senbun.getb(), p));
        ...
        if (i_egaki_dankai >= 3) {
            ...
            if (oc.kyori_senbun(p, moyori_senbun) < d_hantei_haba) {
                ...
                for (int i = 1; i <= entyou_kouho_nbox.getsousuu(); i++) {
                    if (oc.senbun_kousa_hantei(ori_s.get(entyou_kouho_nbox.get_int(i)), moyori_senbun, 0.000001, 0.000001) == 31) {//線分が同じならoc.senbun_kousa_hantei==31
                ...
                if (i_senbun_entyou_mode == 0) {
                    ...
                    for (int i = 1; i <= entyou_kouho_nbox.getsousuu(); i++) {
                        ...
                        if (oc.heikou_hantei(ori_s.get(entyou_kouho_nbox.get_int(i)), moyori_senbun, 0.000001) == 0) { //２つの線分が平行かどうかを判定する関数。oc.heikou_hantei(Tyokusen t1,Tyokusen t2)//0=平行でない
                            ...
                            kousa_ten.set(oc.kouten_motome(ori_s.get(entyou_kouho_nbox.get_int(i)), moyori_senbun));
                            //add_sen =new Senbun(kousa_ten,ori_s.get(entyou_kouho_nbox.get_int(i)).get_tikai_hasi(kousa_ten));
                            add_sen.seta(kousa_ten);
                            add_sen.setb(ori_s.get(entyou_kouho_nbox.get_int(i)).get_tikai_hasi(kousa_ten));
                ...
                if (i_senbun_entyou_mode == 1) {
                    ...
                    int sousuu_old = ori_s.getsousuu();//(1)
                    for (int i = 1; i <= entyou_kouho_nbox.getsousuu(); i++) {
                        Senbun moto_no_sen = new Senbun();
                        moto_no_sen.set(ori_s.get(entyou_kouho_nbox.get_int(i)));
                        Ten p_ten = new Ten();
                        p_ten.set(oc.kouten_motome(moto_no_sen, s_step[1]));

                        if (p_ten.kyori(moto_no_sen.geta()) < p_ten.kyori(moto_no_sen.getb())) {
...
//71 71 71 71 71 71 71 71 71 71 71 71 71 71    i_mouse_modeA==71　;線分延長モード
    ...
    Ten moyori_ten_memo = new Ten();

    //マウス操作(マウスを動かしたとき)を行う関数    //System.out.println("_");
    public void mMoved_A_71(Ten p0) {
        if (i_egaki_dankai == 0) {
            i_dousa_mode = 0;
            mMoved_A_01(p0);
            return;
    ...
    public void mPressed_A_71(Ten p0) {
        ...
        if (p.kyori(moyori_ten_memo) <= d_hantei_haba) {
            i_egaki_dankai = 0;
        }
        ...
        if (i_egaki_dankai == 0) {
            ...
            moyori_ten.set(get_moyori_ten(p));
            moyori_ten_memo.set(moyori_ten);
        ...
        if (i_egaki_dankai == 2) {
            ...
            for (int i = 1; i <= i_egaki_dankai; i++) {
                ...
                if (s_step[i].getnagasa() > 0.00000001) {
                    ...
                    add_senbun.set(s_step[i].geta(), s_step[i].getb());//要注意　s_stepは表示上の都合でアクティヴが0以外に設定されているのでadd_senbunにうつしかえてる20170507
...
// ------------------------------------------------------------
    ...
    // ------------------------------------------------------------
    public void renzoku_orikaesi_new(Ten a, Ten b) {//連続折り返しの改良版。
        orihime_app.repaint();
        ...
        if (e_s_dougubako.get_kousaten_made_nobasi_flg_new(a, b) == 1) {
            ...
            Ten new_b = new Ten();
            new_b.set(oc.sentaisyou_ten_motome(kousaten_made_nobasi_saisyono_senbun.geta(), kousaten_made_nobasi_saisyono_senbun.getb(), a));//２つの点t1,t2を通る直線に関して、点pの対照位置にある点を求める public Ten oc.sentaisyou_ten_motome(Ten t1,Ten t2,Ten p){
...
// ------------------------------------------------------------
    ...
    public void renzoku_orikaesi_1_kaime(Ten a, Ten b) {//連続折り返しの1回目だけここを実施する。連続折り返しの2回目以降はただのrenzoku_orikaesi関数で行う。
...
//29 29 29 29 29 29 29 29  i_mouse_modeA==29正多角形入力	入力 29 29 29 29 29 29 29 29
    ...
    public Ten kouho_ten_A_37(Ten syuusei_ten) {
        ...
        if ((zure_flg == 0) && (syuusei_ten.kyori(moyori_ten) <= d_hantei_haba)) {//最寄点が角度系にのっていて、修正点とも近い場合
            return moyori_ten;
        }
        return syuusei_ten;
    }
...
//--------------------
    ...
    public void mReleased_A_59(Ten p0) {//補助活線と円
        i_egaki_dankai = 0;
...
//58 58 58 58 58 58 58 58 58 58
    ...
    public void mReleased_A_58(Ten p0) {//ここの処理の終わりに fix2(0.001,0.5);　をするのは、元から折線だったものと、補助線から変換した折線との組合せで頻発するT字型不接続を修正するため
        i_egaki_dankai = 0;
...
//--------------------
    ...
    public void mPressed_A_30(Ten p0) {    //マウス操作(i_mouse_modeA==4線_変換　でボタンを押したとき)時の作業
        //Ten p =new Ten();
        p.set(camera.TV2object(p0));
        minrid_30 = -1;
        if (ori_s.mottomo_tikai_senbun_kyori(p) < d_hantei_haba) {//点pに最も近い線分の番号での、その距離を返す	public double mottomo_tikai_senbun_kyori(Ten p)
    ...
    public void mDragged_A_30(Ten p0) {//マウス操作(i_mouse_modeA==4線_変換　でドラッグしたとき)を行う関数
        if (minrid_30 > 0) {
...
//38 38 38 38 38 38 38    i_mouse_modeA==38　;折り畳み可能線入力  qqqqqqqqq
    ...
    //Ten t1 =new Ten();
//マウス操作(ボタンを押したとき)時の作業
    public int mPressed_A_38(Ten p0) {//作業がすべて完了し新たな折線を追加でた場合だけ1を返す。それ以外は0を返す。
        i_kouho_dankai = 0;
        //Ten p =new Ten();
        p.set(camera.TV2object(p0));
        if (i_egaki_dankai == 0) {
        ...
        if (i_step_for38 == 2) {
            ...
            if (oc.kyori_senbun(p, moyori_senbun) >= d_hantei_haba) {//最寄の既存折線が遠くて選択無効の場合
                //moyori_senbun.set(get_moyori_step_senbun(p,1,i_egaki_dankai));
                if (oc.kyori_senbun(p, moyori_step_senbun) < d_hantei_haba) {//最寄のstep_senbunが近い場合
            ...
            if (oc.kyori_senbun(p, moyori_senbun) < d_hantei_haba) {//最寄の既存折線が近い場合
                ...
                Senbun add_sen = new Senbun(kousa_ten, s_step[1].geta(), icol_temp);//20180503変更
                if (add_sen.getnagasa() > 0.00000001) {//最寄の既存折線が有効の場合
                    addsenbun(add_sen);
                    kiroku();
                    i_egaki_dankai = 0;
...
//33 33 33 33 33 33 33 33 33 33 33魚の骨
    ...
    public void mReleased_A_33(Ten p0) {
        if (i_egaki_dankai == 1) {
            ...
            if (p.kyori(moyori_ten) <= d_hantei_haba) {  //マウスで指定した点が、最寄点と近かったときに実施
...
//------
    ...
    public Senbun kousatenmade(Senbun s0) {//s0を点aからb方向に、他の折線と交差するところまで延長する。新しい線分を返す//他の折線と交差しないなら、同じ線分を返す
        Senbun add_sen = new Senbun();
        add_sen.set(s0);
...
//------
    ...
    public Senbun kousatenmade_2(Senbun s0) {//s0を点bからaの反対方向に、他の折線と交差するところまで延長する。新しい線分を返す//他の折線と交差しないなら、同じ線分を返す
        Senbun add_sen = new Senbun();
        add_sen.set(s0);
...
//------
    ...
    public int kouten_ari_nasi(Senbun s0) {//s0を点aからb方向に延長したら、他の折線と交差するなら、１しないなら０を返す。a店での交差する線分は、この関数では交差なしになる。
        Senbun add_sen = new Senbun();
        add_sen.set(s0);
        Ten kousa_ten = new Ten(1000000.0, 1000000.0); //この方法だと、エラーの原因になりうる。本当なら全線分のx_max、y_max以上の点を取ればいい。今後修正予定20161120
        double kousa_ten_kyori = kousa_ten.kyori(add_sen.geta());
...
//64 64 64 64 64 64 64 64 64 64 64 64 64入力した線分に重複している折線を削除する
    ...
    public void mMoved_A_64(Ten p0) {
        mMoved_A_11(p0);
    }//近い既存点のみ表示

    //マウス操作(i_mouse_modeA==64　でボタンを押したとき)時の作業----------------------------------------------------
    public void mPressed_A_64(Ten p0) {
        i_egaki_dankai = 1;
    ...
    public void mDragged_A_64(Ten p0) {
        mDragged_A_11(p0);
    }


    //マウス操作(i_mouse_modeA==64　でボタンを離したとき)を行う関数----------------------------------------------------
    public void mReleased_A_64(Ten p0) {
...
//65 65 65 65 65 65 65 65 65 65 65 65 65入力した線分に重複している折線やX交差している折線を削除する
    ...
    //マウスを動かしたとき
    public void mMoved_A_65(Ten p0) {
        mMoved_m_00b(p0, 5);
    }//マウスで選択できる候補点を表示する。近くに既成の点があるときはその点、無いときはマウスの位置自身が候補点となる。

    //マウスクリック----------------------------------------------------
    public void mPressed_A_65(Ten p0) {
        mPressed_m_00b(p0, 5);
    }

    //マウスドラッグ----------------------------------------------------
    public void mDragged_A_65(Ten p0) {
        mDragged_m_00b(p0, 5);
    }

    //マウス操作(i_mouse_modeA==65　でボタンを離したとき)を行う関数----------------------------------------------------
    public void mReleased_A_65(Ten p0) {
...
//多角形を入力(既存頂点への引き寄せあるが既存頂点が遠い場合は引き寄せ無し)し、何らかの作業を行うセット
    ...
    public void mMoved_takakukei_and_sagyou(Ten p0) {
...
//	public void mMoved_m_002(Ten p0,int i_c) //マウスで選択できる候補点を表示する。近くに既成の点があるときはその点、無いときはマウスの位置自身が候補点となる。
        if (i_kou_mitudo_nyuuryoku == 1) {
            ...
            i_kouho_dankai = 1;
            moyori_ten.set(get_moyori_ten(p));
            if (p.kyori(moyori_ten) > p.kyori(s_step[1].geta())) {
                moyori_ten.set(s_step[1].geta());
            }
    ...
    //マウス操作(ボタンを押したとき)時の作業----------------------------------------------------
    public void mPressed_takakukei_and_sagyou(Ten p0) {
//i_egaki_dankai==0なのはこの操作ボタンを押した直後の段階か、多角形が完成して、その後ボタンを押した後
        ...
        if (i_egaki_dankai == 1) {
            ...
            s_step[i_egaki_dankai].set(moyori_ten, p);
    ...
    public void mDragged_takakukei_and_sagyou(Ten p0) {
...
//mDragged_m_00b(Ten p0,int i_c)
    ...
    //マウス操作(ボタンを離したとき)を行う関数----------------------------------------------------
    public void mReleased_takakukei_and_sagyou(Ten p0, int i_mode) {
        //i_kouho_dankai=0;
        //if(i_takakukei_kansei==0){//ここにくるときは必ずi_takakukei_kansei==0なのでif分は無意味
        ...
        s_step[i_egaki_dankai].setb(moyori_ten);
...
//20201024高密度入力がオンならばapのrepaint（画面更新）のたびにTen kus_sisuu=new Ten(es1.get_moyori_ten_sisuu(p_mouse_TV_iti));で最寄り点を求めているので、この描き職人内で別途最寄り点を求めていることは二度手間になっている。
    ...
    //66 66 66 66 66 多角形を入力し、それに全体が含まれる折線をselectする
    public void mMoved_A_66(Ten p0) {
        mMoved_takakukei_and_sagyou(p0);
    }    //マウス操作(マウスを動かしたとき)を行う関数

    public void mPressed_A_66(Ten p0) {
        mPressed_takakukei_and_sagyou(p0);
    }    //マウス操作でボタンを押したとき)時の作業----------------------------------------------------

    public void mDragged_A_66(Ten p0) {
        mDragged_takakukei_and_sagyou(p0);
    }    //マウス操作(ドラッグしたとき)を行う関数----------------------------------------------------

    public void mReleased_A_66(Ten p0) {
        mReleased_takakukei_and_sagyou(p0, 66);
    }    //マウス操作(ボタンを離したとき)を行う関数----------------------------------------------------


    //67 67 67 67 67 多角形を入力し、それに全体が含まれる折線を折線をunselectする
    public void mMoved_A_67(Ten p0) {
        mMoved_takakukei_and_sagyou(p0);
    }    //マウス操作(マウスを動かしたとき)を行う関数

    public void mPressed_A_67(Ten p0) {
        mPressed_takakukei_and_sagyou(p0);
    }    //マウス操作でボタンを押したとき)時の作業----------------------------------------------------

    public void mDragged_A_67(Ten p0) {
        mDragged_takakukei_and_sagyou(p0);
    }    //マウス操作(ドラッグしたとき)を行う関数----------------------------------------------------

    public void mReleased_A_67(Ten p0) {
...
//68 68 68 68 68 入力した線分に重複している折線やX交差している折線をselectする
    ...
    //マウスを動かしたとき
    public void mMoved_A_68(Ten p0) {
        mMoved_m_00b(p0, 5);
    }//マウスで選択できる候補点を表示する。近くに既成の点があるときはその点、無いときはマウスの位置自身が候補点となる。

    //マウスクリック----------------------------------------------------
    public void mPressed_A_68(Ten p0) {
        mPressed_m_00b(p0, 5);
    }

    //マウスドラッグ----------------------------------------------------
    public void mDragged_A_68(Ten p0) {
        mDragged_m_00b(p0, 5);
    }

    //マウス操作でボタンを離したとき)を行う関数----------------------------------------------------
    public void mReleased_A_68(Ten p0) {
...
//69 69 69 69 69 入力した線分に重複している折線やX交差している折線をunselectする
    ...
    //マウスを動かしたとき
    public void mMoved_A_69(Ten p0) {
        mMoved_m_00b(p0, 5);
    }//マウスで選択できる候補点を表示する。近くに既成の点があるときはその点、無いときはマウスの位置自身が候補点となる。

    //マウスクリック----------------------------------------------------
    public void mPressed_A_69(Ten p0) {
        mPressed_m_00b(p0, 5);
    }

    //マウスドラッグ----------------------------------------------------
    public void mDragged_A_69(Ten p0) {
        mDragged_m_00b(p0, 5);
    }

    //マウス操作でボタンを離したとき)を行う関数----------------------------------------------------
    public void mReleased_A_69(Ten p0) {
...
//63 63 63 外周部の折り畳みチェック
    ...
    public void mMoved_A_63(Ten p0) {
		/* if(i_kou_mitudo_nyuuryoku==1){s_kouho[1].setiactive(3);
...
//icol=7 yellow
    ...
    public void mPressed_A_63(Ten p0) {
        if (i_egaki_dankai == 0) {
            i_O_F_C = 0;
            i_egaki_dankai = i_egaki_dankai + 1;
    ...
    public void mDragged_A_63(Ten p0) {
        if (i_O_F_C == 0) {
            p.set(camera.TV2object(p0));
            s_step[i_egaki_dankai].setb(p);
        }
...
*/
    ...
    public void mReleased_A_63(Ten p0) {
...
//16 16 16 16 16 16    i_mouse_modeA==16　;角度系モード
    ...
    public void mPressed_A_16(Ten p0) {
        ...
        if (i_egaki_dankai == 2 + (honsuu) + 1) {
            ...
            moyori_senbun.set(get_moyori_senbun(p));
            if (oc.kyori_senbun(p, moyori_senbun) >= d_hantei_haba) {//最寄折線が遠かった場合
                i_egaki_dankai = 0;
                return;
            }
    ...
    public void mPressed_A_18(Ten p0) {
        ...
        if (i_egaki_dankai == 2 + (honsuu)) {
            ...
            if (oc.kyori_senbun(p, moyori_step_senbun) < d_hantei_haba) {
                ...
                moyori_senbun.set(get_moyori_senbun(p));
                if (oc.kyori_senbun(p, moyori_senbun) < d_hantei_haba) {//最寄折線が近い場合
                    if (oc.heikou_hantei(moyori_step_senbun, moyori_senbun, 0.000001) == 0) {//最寄折線が最寄step折線と平行の場合は除外
...
//14 14 14 14 14 14 14 14 14    i_mouse_modeA==14　;V追加モード
    ...
    public void mPressed_A_14(Ten p0) {
        ...
        Senbun mts = new Senbun(ori_s.geta(mts_id), ori_s.getb(mts_id));//mtsは点pに最も近い線分
...
//24 24 24 24 24
    ...
    public void mReleased_A_24(Ten p0) {//ここの処理の終わりに fix2(0.001,0.5);　をするのは、元から折線だったものと、補助線から変換した折線との組合せで頻発するT字型不接続を修正するため
        i_egaki_dankai = 0;
...
//25 25 25 25 25
    ...
    public void mReleased_A_25(Ten p0) {//ここの処理の終わりに fix2(0.001,0.5);　をするのは、元から折線だったものと、補助線から変換した折線との組合せで頻発するT字型不接続を修正するため
        i_egaki_dankai = 0;
...
//46 46 46 46 46 46 46 46 46   i_mouse_modeA==46　;反転入力モード。
    ...
    public void mPressed_A_46(Ten p0) {
        ...
        if (i_egaki_dankai + i_en_egaki_dankai == 0) {
            ...
            if (oc.kyori_senbun(p, moyori_senbun) < oc.kyori_ensyuu(p, moyori_ensyuu)) {//線分の方が円周より近い
                i_egaki_dankai = 0;
                i_en_egaki_dankai = 0;
                if (oc.kyori_senbun(p, moyori_senbun) > d_hantei_haba) {
                    return;
