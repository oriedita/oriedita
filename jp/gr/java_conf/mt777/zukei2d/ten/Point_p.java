...
public class Ten {//点の座標や方向ベクトルなどをあらわすときに用いる
	...
	public Ten(Ten p){set(p);}//コンストラクタ 
	public Ten(){x=0;y=0;}//コンストラクタ
	public Ten(double i,double j){x=i;y=j;} //コンストラクタ
	public Ten(double a,Ten p,double b,Ten q){x=a*p.getx()+b*q.getx();y=a*p.gety()+b*q.gety();} //コンストラクタ


  
	public void set(Ten p){ x=p.getx();y=p.gety();}
	//public void set(Ten p){double dx,dy; dx=p.getx();dy=p.gety();x=dx;y=dy;}
	public void set(double i,double j){ x=i;y=j;}
	public void set(double a,Ten p,double b,Ten q){x=a*p.getx()+b*q.getx();y=a*p.gety()+b*q.gety();}
