...
public class MouseHandlerAxiom5 extends StepMouseHandler<Axiom5Step> {
    ...
    public void drawPreview(Graphics2D g2, Camera camera, DrawingSettings settings) {
        ...
        DrawingUtil.drawStepVertex(g2, pivotPoint, d.getLineColor(), camera);
        DrawingUtil.drawLineStep(g2, indicator1, camera, settings.getLineWidth());
        DrawingUtil.drawLineStep(g2, indicator2, camera, settings.getLineWidth());
    ...
    private void move_drag_select_pivot_point(Point p) {
        if (p.distance(d.getClosestPoint(p)) < d.getSelectionDistance()
                && OritaCalc.determineLineSegmentDistance(d.getClosestPoint(p),
                        new LineSegment(targetPoint, targetPoint)) > Epsilon.UNKNOWN_1EN7
                && !(OritaCalc.isPointWithinLineSpan(d.getClosestPoint(p), targetSegment)
    ...
    private void move_drag_select_destination_or_indicator(Point p) {
        ...
        double minDistance = Collections
                .min(Arrays.asList(indicator1Distance, indicator2Distance, targetSegmentDistance, normalDistance));
    ...
    private Axiom5Step release_select_destination_or_indicator(Point p) {
        if (OritaCalc.determineLineSegmentDistance(p, indicator1) < d.getSelectionDistance() ||
            ...
            LineSegment s = OritaCalc.determineLineSegmentDistance(p, indicator1) < OritaCalc
                    .determineLineSegmentDistance(p, indicator2)
                            ? indicator1
                            : indicator2;
        ...
        if (Math.abs(length_a - radius) < Epsilon.UNKNOWN_1EN7) {
            ...
            if (OritaCalc.isPointWithinLineSpan(targetPoint, projectionLine)) {
                if (OritaCalc.distance(projectionPoint, targetPoint) < Epsilon.UNKNOWN_1EN7) {
                    ...
                    indicator1 = OritaCalc.fullExtendUntilHit(d.getFoldLineSet(),
                            new LineSegment(midPoint,
                                    OritaCalc.findProjection(OritaCalc.moveParallel(projectionLine, -1.0), midPoint),
                                    LineColor.PURPLE_8));
                    indicator2 = OritaCalc.fullExtendUntilHit(d.getFoldLineSet(),
                            new LineSegment(midPoint,
                                    OritaCalc.findProjection(OritaCalc.moveParallel(projectionLine, 1.0), midPoint),
                                    LineColor.PURPLE_8));
                    return;
            ...
            if (OritaCalc.isLineSegmentParallel(new LineSegment(pivotPoint, targetPoint),
                    projectionLine) == OritaCalc.ParallelJudgement.NOT_PARALLEL) {
                s = OritaCalc.fullExtendUntilHit(d.getFoldLineSet(), new LineSegment(pivotPoint,
                        OritaCalc.center(pivotPoint, targetPoint, projectionPoint), LineColor.PURPLE_8));
            } else {
                s = OritaCalc.fullExtendUntilHit(d.getFoldLineSet(),
                        new LineSegment(pivotPoint, projectionPoint, LineColor.PURPLE_8));
            }
            indicator1 = s;
            indicator2 = s;
            ...
            // Length of the last segment of a right triangle (circle center, projection
            // point, and the ultimate guiding point for indicators)
    ...
    private LineSegment processProjectedLineOfIndicator(Point pivot, Point projectPoint, double length) {
        ...
        LineSegment line = new LineSegment(projectPoint,
                OritaCalc.findProjection(OritaCalc.moveParallel(projectLine, length), projectPoint));
    ...
    private Point processCenter(Point pivot, LineSegment l1, LineSegment l2) {
        ...
        // ------------------O-------------------
        // common point ⤴
        if (OritaCalc.isLineSegmentParallel(new StraightLine(l1.determineFurthestEndpoint(pivot), pivot),
                new StraightLine(pivot,
                        l2.determineFurthestEndpoint(pivot))) == OritaCalc.ParallelJudgement.PARALLEL_EQUAL) {
            ...
            return OritaCalc.center(l1.determineFurthestEndpoint(pivot), l2.determineFurthestEndpoint(pivot),
                    seg.determineFurthestEndpoint(pivot));
    ...
    private Pair<LineSegment, LineSegment> processPivotWithinSegmentSpan(LineSegment l1, LineSegment l2,
            LineSegment targetSegment, Point pivot) {
        ...
        if (OritaCalc.isPointWithinLineSpan(pivot, targetSegment)) {
            ...
            boolean isOutsideSegmentA = targetSegment.determineLength() > OritaCalc.distance(targetSegment.getA(),
                    pivot) &&
                    OritaCalc.distance(targetSegment.getB(), pivot) > targetSegment.determineLength();
            boolean isOutsideSegmentB = targetSegment.determineLength() > OritaCalc.distance(targetSegment.getB(),
                    pivot) &&
            ...
            l1 = (new LineSegment(pivot, isOutsideSegmentA ? OritaCalc.point_rotate(pivot, targetSegment.getB(), 180)
                    : targetSegment.getA()));

            // pivot within span outside B
            l2 = (new LineSegment(pivot, isOutsideSegmentB ? OritaCalc.point_rotate(pivot, targetSegment.getA(), 180)
                    : targetSegment.getB()));
            return new ImmutablePair<>(l1, l2);
        }
        return new ImmutablePair<>(l1, l2);
    ...
    private void determineIndicators(LineSegment l, LineSegment l1, LineSegment l2, Point center, Point center1,
            Point center2, Point target, LineSegment targetSegment, Point pivot) {
        if (OritaCalc.distance(center1, OritaCalc.findProjection(targetSegment, center1)) > Epsilon.UNKNOWN_1EN7 ||
            ...
            if (!OritaCalc.isPointWithinLineSpan(target, targetSegment)) {
                ...
                indicator1 = OritaCalc.fullExtendUntilHit(d.getFoldLineSet(),
                        new LineSegment(center, center1, LineColor.PURPLE_8));
                indicator2 = OritaCalc.fullExtendUntilHit(d.getFoldLineSet(),
                        new LineSegment(center, center2, LineColor.PURPLE_8));
                return;
            }
            ...
            if (OritaCalc.isLineSegmentParallel(l1, l) == OritaCalc.ParallelJudgement.PARALLEL_EQUAL) {
                ...
                LineSegment s = OritaCalc.fullExtendUntilHit(d.getFoldLineSet(),
                        new LineSegment(center, center2, LineColor.PURPLE_8));
                indicator1 = s;
                indicator2 = s;
                return;
            ...
            indicator1 = OritaCalc.fullExtendUntilHit(d.getFoldLineSet(), new LineSegment(pivot,
                    OritaCalc.findProjection(OritaCalc.moveParallel(l1, 1.0), pivot), LineColor.PURPLE_8));
            indicator2 = OritaCalc.fullExtendUntilHit(d.getFoldLineSet(), new LineSegment(pivot,
                    OritaCalc.findProjection(OritaCalc.moveParallel(l2, -1.0), pivot), LineColor.PURPLE_8));
        }
    }
}
