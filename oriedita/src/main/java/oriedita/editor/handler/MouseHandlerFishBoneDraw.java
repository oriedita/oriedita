...
public class MouseHandlerFishBoneDraw extends StepMouseHandler<FishBoneDrawStep> {
    ...
    public MouseHandlerFishBoneDraw() {
        ...
        steps.addNode(ObjCoordStepNode.createNode(FishBoneDrawStep.CLICK_DRAG_POINT, this::move_click_drag_point, (p) -> {
    ...
    private FishBoneDrawStep release_click_drag_point(Point p) {
        ...
        double dx = (dragSegment.determineAX() - dragSegment.determineBX()) * d.getGrid().getGridWidth()
                / dragSegment.determineLength();
        double dy = (dragSegment.determineAY() - dragSegment.determineBY()) * d.getGrid().getGridWidth()
                / dragSegment.determineLength();
        ...
        for (int i = 0; i <= (int) Math.floor(dragSegment.determineLength() / d.getGrid().getGridWidth()); i++) {
            ...
            if (d.getFoldLineSet().closestLineSegmentDistanceExcludingParallel(pxy,
                    dragSegment) <= Epsilon.UNKNOWN_0001) {
                continue;
            }
    ...
    public int kouten_ari_nasi(LineSegment s0) {// If s0 is extended from the point a to the b direction and intersects
                                                // with another polygonal line, 0 is returned if it is not 1. The
                                                // intersecting line segments at the a store have no intersection with
                                                // this function.
        ...
        for (var ls : d.getFoldLineSet().getLineSegmentsIterable()) {
            i_intersection_flg = tyoku1.lineSegment_intersect_reverse_detail(ls);// 0 = This straight line does not
                                                                                 // intersect a given line segment, 1 =
                                                                                 // X type intersects, 2 = T type
                                                                                 // intersects, 3 = Line segment is
