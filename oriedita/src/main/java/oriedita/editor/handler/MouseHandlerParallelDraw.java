...
public class MouseHandlerParallelDraw extends StepMouseHandler<ParallelDrawStep> {
    ...
    // t_taisyou.set(oc.sentaisyou_ten_motome(lineStep.get(1).geta(),line_step[3].geta(),lineStep.get(0).geta()));
    ...
    private void move_select_destination(Point p) {
        if (OritaCalc.determineLineSegmentDistance(p, d.getClosestLineSegment(p)) < d.getSelectionDistance()
                && OritaCalc.isLineSegmentParallel(parallelSegment,
    ...
    /**
     * When i_egaki_dankai is i_e_d, add a temporary fold line s_step [i_e_d + 1]
     * (color is i colo) that
     * extends the line segment s_o to the intersection of s_k while keeping Point a
     * as it is. Returns 1 on success,
