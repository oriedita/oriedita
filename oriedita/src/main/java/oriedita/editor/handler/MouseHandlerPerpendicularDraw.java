...
public class MouseHandlerPerpendicularDraw extends StepMouseHandler<PerpendicularDrawStep> {
    ...
    public MouseHandlerPerpendicularDraw() {
        ...
        steps.addNode(ObjCoordStepNode.createNode_MD_R(PerpendicularDrawStep.SELECT_TARGET_POINT,
                this::move_drag_select_target_point, this::release_select_target_point));
        steps.addNode(ObjCoordStepNode.createNode_MD_R(PerpendicularDrawStep.SELECT_PERPENDICULAR_SEGMENT,
                this::move_drag_select_perpendicular_segment, this::release_select_perpendicular_segment));
        steps.addNode(ObjCoordStepNode.createNode_MD_R(PerpendicularDrawStep.SELECT_DESTINATION_OR_INDICATOR,
    ...
    private PerpendicularDrawStep release_select_perpendicular_segment(Point p) {
        ...
        if (OritaCalc.isPointWithinLineSpan(targetPoint, perpendicularSegment)) {
            indicator = OritaCalc.fullExtendUntilHit(d.getFoldLineSet(),
                    new LineSegment(targetPoint,
                            OritaCalc.findProjection(OritaCalc.moveParallel(perpendicularSegment, 1.0), targetPoint),
                            LineColor.PURPLE_8));
            indicator = OritaCalc.fullExtendUntilHit(d.getFoldLineSet(),
        ...
        LineSegment nonBaseResultLine = new LineSegment(targetPoint,
                OritaCalc.findProjection(OritaCalc.lineSegmentToStraightLine(perpendicularSegment), targetPoint),
                d.getLineColor());
