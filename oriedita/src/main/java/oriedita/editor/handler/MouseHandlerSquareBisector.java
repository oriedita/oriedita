...
public class MouseHandlerSquareBisector extends StepMouseHandler<SquareBisectorStep> {
    ...
    public MouseHandlerSquareBisector() {
        ...
        steps.addNode(ObjCoordStepNode.createNode_MD_R(SquareBisectorStep.SELECT_2L_OR_3P, this::move_drag_select_2L_or_3P,
                this::release_select_2L_or_3P));
        steps.addNode(ObjCoordStepNode.createNode_MD_R(SquareBisectorStep.SELECT_3P, this::move_drag_select_3P,
                this::release_select_3P));
        steps.addNode(ObjCoordStepNode.createNode_MD_R(SquareBisectorStep.SELECT_DESTINATION_3P, this::move_drag_destination_3P,
                this::release_destination_3P));
        steps.addNode(ObjCoordStepNode.createNode_MD_R(SquareBisectorStep.SELECT_2L, this::move_drag_select_2L,
                this::release_select_2L));
        steps.addNode(ObjCoordStepNode.createNode_MD_R(SquareBisectorStep.SELECT_DESTINATION_2L_NP,
                this::move_drag_select_destination_2L_NP, this::release_select_destination_2L_NP));
        steps.addNode(ObjCoordStepNode.createNode_MD_R(SquareBisectorStep.SELECT_DESTINATION_2L_P,
                this::move_drag_select_destination_2L_P, this::release_select_destination_2L_P));
    }
    ...
    private SquareBisectorStep release_destination_3P(Point p) {
        ...
        if (OritaCalc.isLineSegmentParallel(add_sen2,
                destinationSegment_3P) != OritaCalc.ParallelJudgement.NOT_PARALLEL) {
            return SquareBisectorStep.SELECT_DESTINATION_3P;
        }
    ...
    private SquareBisectorStep release_select_destination_2L_NP(Point p) {
        if (destinationSegment_2L_NP == null)
        ...
        Point center = OritaCalc.center(intersection, segmentsList_2L.get(0).determineFurthestEndpoint(intersection),
                segmentsList_2L.get(1).determineFurthestEndpoint(intersection));
        ...
        if (OritaCalc.isLineSegmentParallel(tempBisect,
                destinationSegment_2L_NP) != OritaCalc.ParallelJudgement.NOT_PARALLEL) {
            return SquareBisectorStep.SELECT_DESTINATION_2L_NP;
        }
    ...
    private void move_drag_select_destination_2L_P(Point p) {
        if (OritaCalc.determineLineSegmentDistance(p, d.getClosestLineSegment(p)) < d.getSelectionDistance()
                && OritaCalc.isLineSegmentParallel(d.getClosestLineSegment(p),
                        segmentsList_2L.get(1)) == OritaCalc.ParallelJudgement.NOT_PARALLEL) {
            destinationSegmentsList_2L_P.set(counter_2L_P, d.getClosestLineSegment(p).withColor(LineColor.ORANGE_4));
        } else
            destinationSegmentsList_2L_P.set(counter_2L_P, null);
    ...
    private SquareBisectorStep release_select_destination_2L_P(Point p) {
        ...
        if (destinationSegmentsList_2L_P.get(counter_2L_P) == null)
            return SquareBisectorStep.SELECT_DESTINATION_2L_P;
        counter_2L_P++;
        if (counter_2L_P != 2)
            return SquareBisectorStep.SELECT_DESTINATION_2L_P;
        if (OritaCalc.isLineSegmentParallel(destinationSegmentsList_2L_P.get(0),
                destinationSegmentsList_2L_P.get(1)) == OritaCalc.ParallelJudgement.PARALLEL_EQUAL) {
            reset();
            return SquareBisectorStep.SELECT_2L_OR_3P;
        }
    ...
    private SquareBisectorStep checkIfParallel() {
        if (OritaCalc.isLineSegmentParallel(segmentsList_2L.get(0), segmentsList_2L.get(1),
                Epsilon.UNKNOWN_1EN4) == OritaCalc.ParallelJudgement.NOT_PARALLEL) {
            return SquareBisectorStep.SELECT_DESTINATION_2L_NP;
        } else {
