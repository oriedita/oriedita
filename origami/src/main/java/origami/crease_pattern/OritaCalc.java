...
public class OritaCalc {
    // Find the position of the projection of the point p on the straight line t
    // (the position on the straight line t closest to the point p).
    public static Point findProjection(StraightLine t, Point p) {
        StraightLine t1 = t.orthogonalize(p);// Find the straight line u1 that passes through the point p1 and is
                                             // orthogonal to t.
        return findIntersection(t, t1);
    }

    // Find the position of the projection of the point p on the straight line t
    // passing through the points P0 and P1 (the position on the straight line t
    ...
    // Find the position of the projection of the point p on the straight line t
    // including the line segment s0 (the position on the straight line t closest to
    // the point p).
    public static Point findProjection(LineSegment s0, Point p) {
        return findProjection(s0.getA(), s0.getB(), p);
    }

    // A function that determines whether two points are in the same position (true)
    // or different (false) -------------------------------- -
    public static boolean equal(Point p1, Point p2) {
        return equal(p1, p2, Epsilon.POINT);
    }

    public static boolean equal(Point p1, Point p2, double r) {// r is the error tolerance. Strict judgment if r is
                                                               // negative.
        // Strict judgment。
        if (r <= 0.0) {
    ...
    // Function to find the distance (integer) between two
    // points----------------------------------------------------
    public static double distance(Point p0, Point p1) {
        return p0.distance(p1);
    }

    // A function that finds the angle between the vector ab and the x-axis by
    // specifying a and b between two points. If a = b, return -10000.0
    // ----------------------------------------- -----------
    ...
    // A function that specifies the line segment and finds the angle between the
    // vector ab and the x-axis. If a = b, return -10000.0
    // ----------------------------------------------------
    public static double angle(LineSegment s) {
        return angle(s.getA(), s.getB());
    }

    // A function that returns 2 if the point pa is in a rectangle containing two
    // line segments that is orthogonal to the line segment ending at the two points
    // p1 and p2 at the points p1 and p2.
    public static int isInside(Point p1, Point pa, Point p2) {
        StraightLine u1 = new StraightLine(p1, p2).orthogonalize(p1);// Find the straight line u1 that passes through
                                                                     // the point p1 and is orthogonal to t.
        StraightLine u2 = new StraightLine(p1, p2).orthogonalize(p2);// Find the straight line u2 that passes through
                                                                     // the point p2 and is orthogonal to t.
    ...
    // A function that returns 2 if the point pa is in a rectangle containing two
    // line segments that is orthogonal to the line segment ending at the two points
    // p1 and p2 at the points p1 and p2. This is considered to be inside the
    // rectangle even if it protrudes a little.
    // Specifically, when determining whether there is a point inside the line
    // segment, if the point is slightly outside the line segment, it is judged to
    // be sweet if it is inside the line segment. When drawing a crease pattern with
    // a drawing craftsman, if you do not use this sweet one, the intersection
    // division of the T-shaped line segment will fail
    // But for some reason, using this sweeter one for folding estimation seems to
    // result in an infinite loop, which doesn't work. This exact elucidation is
    // unresolved 20161105
    ...
    /**
     * A function that determines where the point p is close to the specified line
     * segment (within r) ------------------------ ---------
     * 0 = not close, 1 = close to point a, 2 = close to point b, 3 = close to
     * handle
     */
    ...
    // Function to find the distance between the point p0 and the line segment with
    // the two points p1 and p2 at both ends ---------------------------
    // -------------------------
    public static double determineLineSegmentDistance(Point p0, Point p1, Point p2) {
        ...
        StraightLine u = t.orthogonalize(p0);// Find a straight line u that passes through the point p0 and is
                                             // orthogonal to t.
        ...
        return Math.min(distance(p0, p1), distance(p0, p2));// When the intersection of t and u is not between p1 and
                                                            // p2.
    }

    // A function that finds the distance between the point p0 and the line segment
    // s ----------------------------------- -----------------
    ...
    // ---------------------------------- ------------------ ------------------
    // 0 = Do not intersect,
    // 1 = Two line segments are not parallel and intersect at one point in a
    // crossroads shape,
    // 2nd generation = Two line segments are not parallel and intersect in a
    // T-junction or dogleg shape at one point
    ...
    // Note! If p1 and p2 are the same, or p3 and p4 are the same, the result will
    // be strange,
    // This function itself does not have a check mechanism, so it may be difficult
    // to notice.
    ...
    public static LineSegment.Intersection determineLineSegmentIntersection(LineSegment s1, LineSegment s2, double rhit,
            double rhei) { // r_hitosii and r_heikouhantei are the allowable degree of deviation between
                           // hitosii and heikou_hantei
        ...
        if (isLineSegmentParallel(t1, t2, rhei) == ParallelJudgement.NOT_PARALLEL) { // Two straight lines are not
                                                                                     // parallel
        ...
        if (isLineSegmentParallel(t1, t2, rhei) == ParallelJudgement.PARALLEL_NOT_EQUAL) { // Two straight lines are
                                                                                           // parallel and y-intercept
                                                                                           // does not match
            return LineSegment.Intersection.NO_INTERSECTION_0;
        }
        ...
        if (isLineSegmentParallel(t1, t2, rhei) == ParallelJudgement.PARALLEL_EQUAL) {
            ...
            } // The P2 side of the line segment (p1, p2) and the P3 side of the line segment
              // (p3, p4) partially overlap.
            if ((isInside(p1, p4, p2) == 2) && (isInside(p4, p2, p3) == 2)) {
                return LineSegment.Intersection.PARALLEL_S1_END_OVERLAPS_S2_END_372;
            } // The P2 side of the line segment (p1, p2) and the P4 side of the line segment
              // (p4, p3) partially overlap.

            if ((isInside(p3, p1, p4) == 2) && (isInside(p1, p4, p2) == 2)) {
                return LineSegment.Intersection.PARALLEL_S1_START_OVERLAPS_S2_END_373;
            } // The P4 side of the line segment (p3, p4) and the P1 side of the line segment
              // (p1, p2) partially overlap.
            if ((isInside(p4, p1, p3) == 2) && (isInside(p1, p3, p2) == 2)) {
                return LineSegment.Intersection.PARALLEL_S1_START_OVERLAPS_S2_START_374;
            } // The P3 side of the line segment (p4, p3) and the P1 side of the line segment
              // (p1, p2) partially overlap.
    ...
    // The sweet part of senbun_kousa_hantei_amai is that if ((hakononaka (p1, pk,
    // p2)> = 1) && (hakononaka (p3, pk, p4)> = 1), which is the premise of return
    // 21 to return 28. )) Instead of
    // (hakononaka_amai (p1, pk, p2)> = 1) && (hakononaka_amai (p3, pk, p4) is used.
    // Hakononaka_amai is
    // A function that returns 2 if the point pa is in a rectangle containing two
    // line segments that is orthogonal to the line segment with the two points p1
    // and p2 as the end points at the points p1 and p2. This is considered to be
    // inside the rectangle even if it protrudes a little.
    // Specifically, when determining whether there is a point inside the line
    // segment, if the point is slightly outside the line segment, it is judged to
    // be sweet if it is inside the line segment. When drawing a crease pattern with
    // a drawing craftsman, if you do not use this sweet one, the intersection
    ...
    public static LineSegment.Intersection determineLineSegmentIntersectionSweet(LineSegment s1, LineSegment s2,
            double rhit, double rhei) { // r_hitosiiとr_heikouhanteiは、hitosiiとheikou_hanteiのずれの許容程度
        double x1max = s1.determineAX();
        double x1min = s1.determineAX();
        double y1max = s1.determineAY();
        double y1min = s1.determineAY();
    ...
    // ---------------------------
    public static LineSegment extendToIntersectionPoint_2(FoldLineSet foldLineSet, LineSegment s0) {// Extend s0 from
                                                                                                    // point b in the
                                                                                                    // opposite
                                                                                                    // direction of a to
                                                                                                    // the point where
        ...
        for (var s : foldLineSet.getLineSegmentsIterable()) {
            ...
            i_lineSegment_intersection_flg = OritaCalc.determineLineSegmentIntersectionSweet(s0, s,
                    Epsilon.UNKNOWN_1EN5, Epsilon.UNKNOWN_1EN5);// 20180408なぜかこの行の様にs0のままだと、i_senbun_kousa_flgがおかしくならない。
            ...
            if (i_intersection_flg == StraightLine.Intersection.INCLUDED_3
                    && i_lineSegment_intersection_flg != LineSegment.Intersection.PARALLEL_EQUAL_31) {
                kousa_point = s.getA();
                if (kousa_point.distance(add_sen.getA()) > Epsilon.UNKNOWN_1EN5) {
                    if (kousa_point.distance(add_sen.getA()) < kousa_point_distance) {
                        double d_kakudo = OritaCalc.angle(add_sen.getA(), add_sen.getB(), add_sen.getA(), kousa_point);
    ...
    public static LineSegment fullExtendUntilHit(FoldLineSet foldLineSet, LineSegment s0) {
        ...
        temp = new LineSegment(point, temp.determineFurthestEndpoint(point), s0.getColor());
        return temp;
    }
    ...
    public static ParallelJudgement isLineSegmentParallel(StraightLine t1, StraightLine t2, double r) {// rは誤差の許容度。rが負なら厳密判定。
        // 0 = not parallel, 1 = parallel and 2 straight lines do not match, 2 =
        // parallel and 2 straight lines match
        ...
        if (r > 0) {
            ...
            if (Math.abs(a1 * b2 - a2 * b1) < r) {
                ...
                // 原点（0、0）と各直線との距離を比較
                double kyoriT = t2.calculateDistance(t1.findProjection(new Point(0.0, 0.0)));// t1上の点とt2との距離//t1.kage_motome(new
                                                                                             // Ten(0.0,0.0))
                                                                                             // は点（0,0）のt1上の影を求める（t1上の点ならなんでもいい）//20181115修正
    ...
    public static StraightLine lineSegmentToStraightLine(LineSegment s) {// Get a straight line containing a line
                                                                         // segment
        return new StraightLine(s.getA(), s.getB());
    }

    // A function that finds the intersection of two line segments as a straight
    // line. Even if it does not intersect as a line segment, it returns the
    // intersection when it intersects as a straight line
    ...
    // A function that considers a line segment as a straight line and finds the
    // intersection with another straight line. Even if it does not intersect as a
    // line segment, it returns the intersection when it intersects as a straight
    // line
        ...
        while (!pathIterator.isDone()) {
            ...
            switch (segmentType) {
                ...
                case PathIterator.SEG_LINETO:
                    ...
                    currentPoint.setLocation(coords[0], coords[1]);
                    if (lineSegment.intersectsLine(new Line2D.Double(lastPoint, currentPoint)))
                        return true;
                    break;
                case PathIterator.SEG_CLOSE:
                    currentPoint.setLocation(coords[0], coords[1]);
                    if (lineSegment.intersectsLine(new Line2D.Double(currentPoint, firstPoint)))
                        return true;
                    break;
            }
            pathIterator.next();
    ...
    public static boolean isLineSegmentContainedInPath(Path2D path, Line2D lineSegment) {
        if (!path.contains(lineSegment.getP1()) || !path.contains(lineSegment.getP2()))
            return false;
        ...
        while (!pathIterator.isDone()) {
            ...
            switch (segmentType) {
                ...
                case PathIterator.SEG_LINETO:
                    ...
                    if (lineSegment.intersectsLine(pathSegment))
                        return false;
                    break;
                case PathIterator.SEG_CLOSE:
                    break;
    ...
    // A function that moves a line segment in parallel to the side (returns a new
    // line segment without changing the original line segment)
    ...
    // ------------------------------------
    // A function that returns a point obtained by rotating point b by d degrees
    // around point a (returns a new point without changing the original point)
    ...
    // ------------------------------------
    // A function that rotates point b by d degrees around point a and returns a
    // point whose ab distance is r times (returns a new point without changing the
    // original point)
    ...
    public static LineSegment lineSegment_rotate(LineSegment s0, double d) {
        ...
        double bx1 = Mcd * (s0.determineBX() - s0.determineAX()) - Msd * (s0.determineBY() - s0.determineAY())
                + s0.determineAX();
        double by1 = Msd * (s0.determineBX() - s0.determineAX()) + Mcd * (s0.determineBY() - s0.determineAY())
                + s0.determineAY();
    ...
    public static LineSegment lineSegment_rotate(LineSegment s0, double d, double r) {
        ...
        double bx1 = r * (Mcd * (s0.determineBX() - s0.determineAX()) - Msd * (s0.determineBY() - s0.determineAY()))
                + s0.determineAX();
        double by1 = r * (Msd * (s0.determineBX() - s0.determineAX()) + Mcd * (s0.determineBY() - s0.determineAY()))
                + s0.determineAY();
    ...
    /**
     * A function that returns a line segment obtained by multiplying the line
     * segment ab by lengthMultiplier with a as the center
    ...
    // A function to find the line segment B at the control position of the line
    // segment A with the line segment J as the axis.
    ...
    // A function that finds a point at the control position of point p with respect
    // to a straight line passing through two points t1 and t2.
    public static Point findLineSymmetryPoint(Point t1, Point t2, Point p) {
        ...
        // Find the straight line s2 that passes through the point p and is orthogonal
        // to s1.
    ...
    // A function that keeps the angle greater than -180.0 degrees and less than
    // 180.0 degrees
    ...
    public static Point internalDivisionRatio(Point a, Point b, double d_internalDivisionRatio_s,
        ...
        if (d_internalDivisionRatio_s == 0.0) {
            ...
            if (d_internalDivisionRatio_t == 0.0) {
                ...
                double nx = (d_internalDivisionRatio_t * s_ab.determineAX()
                        + d_internalDivisionRatio_s * s_ab.determineBX())
                        / (d_internalDivisionRatio_s + d_internalDivisionRatio_t);
                double ny = (d_internalDivisionRatio_t * s_ab.determineAY()
                        + d_internalDivisionRatio_s * s_ab.determineBY())
                        / (d_internalDivisionRatio_s + d_internalDivisionRatio_t);
    ...
    // -------------------------------
    public static Circle.Intersection circle_to_circle_intersection(Circle c1, Circle c2) {
        double centerDist = c1.determineCenter().distance(c2.determineCenter());
        double tangentDist = c1.getR() + c2.getR();

        if (Math.abs(centerDist - tangentDist) < Epsilon.UNKNOWN_1EN6) {
            return Circle.Intersection.TANGENT;
        } else if (centerDist < tangentDist) {
            return Circle.Intersection.INTERSECT;
        } else
            return Circle.Intersection.NO_INTERSECTION;
    }
    ...
    public static LineSegment circle_to_circle_no_intersection_wo_musubu_lineSegment(Circle e1, Circle e2) {
        ...
        // double length_a=kyori(intersection_t0t1,e1.get_tyuusin());
        // //t0とt1の交点からe1の中心までの長さ
        double length_b = Math.sqrt(e1.getR() * e1.getR() - length_a * length_a); // t0とt1の交点からe1とe2の交点までの長さ
        // t0と平行な方向ベクトルは(t0.getb() , -t0.geta())
        // t0と平行な方向ベクトルで長さがlength_bのものは(t0.getb()*length_b/Math.sqrt(t0.getb()*t0.getb()+
        // t0.geta()*t0.geta() ) , -t0.geta()*length_b/Math.sqrt(t0.getb()*t0.getb()+
        // t0.geta()*t0.geta() ))

        return new LineSegment(
                intersection_t0t1.getX()
                        + t0.getB() * length_b / Math.sqrt(t0.getB() * t0.getB() + t0.getA() * t0.getA()),
                intersection_t0t1.getY()
                        - t0.getA() * length_b / Math.sqrt(t0.getB() * t0.getB() + t0.getA() * t0.getA()),
                intersection_t0t1.getX()
                        - t0.getB() * length_b / Math.sqrt(t0.getB() * t0.getB() + t0.getA() * t0.getA()),
                intersection_t0t1.getY()
                        + t0.getA() * length_b / Math.sqrt(t0.getB() * t0.getB() + t0.getA() * t0.getA()));
    ...
    public static LineSegment circle_to_straightLine_no_intersect_wo_connect_LineSegment(Circle e1, StraightLine t0) {
        ...
        double length_b = Math.sqrt(e1.getR() * e1.getR() - length_a * length_a); // t0とt1の交点からe1とe2の交点までの長さ
        // t0と平行な方向ベクトルは(t0.getb() , -t0.geta())
        // t0と平行な方向ベクトルで長さがnagasa_bのものは(t0.getb()*length_b/Math.sqrt(t0.getb()*t0.getb()+
        // t0.geta()*t0.geta() ) , -t0.geta()*length_b/Math.sqrt(t0.getb()*t0.getb()+
        // t0.geta()*t0.geta() ))
    ...
    // Function to find the distance between the point p0 and the circumference of
    // the circle e0 ------------------------------- ---------------------
    // ---------------------
    ...
    // --------------------------------------------------------
    public static boolean isLineSegmentOverlapping(LineSegment s1, LineSegment s2) {// false do not overlap. true
                                                                                    // overlaps. 20201012 added
     ...
     */
    public static boolean isPointWithinLineSpan(Point p0, LineSegment s0) { // Check if point p0 is within the span of
                                                                            // segment s0
