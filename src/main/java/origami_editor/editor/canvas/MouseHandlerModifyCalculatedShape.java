...
public class MouseHandlerModifyCalculatedShape implements MouseModeHandler {
    ...
    public void foldedFigure_operation_mouse_off_1(Point p) {//折り上がり図操作でマウスの左ボタンを離したときの作業
        ...
        if (i_nanini_near == 2) {
            app.OZ.foldedFigure.displayStyle = app.OZ.foldedFigure.display_flg_backup;//20180216

            app.OZ.wireFrame_worker_drawer2.mReleased_selectedPoint_move_with_camera(move_previous_selection_point, p_m_left_on, p, app.OZ.foldedFigure.ip4);
            if (p_m_left_on.distance(p) > Epsilon.UNKNOWN_1EN7) {
                app.OZ.record();
                app.OZ.foldedFigure.estimationStep = FoldedFigure.EstimationStep.STEP_2;

                if (app.OZ.foldedFigure.displayStyle == FoldedFigure.DisplayStyle.WIRE_2) {
                }

                if (app.OZ.foldedFigure.displayStyle == FoldedFigure.DisplayStyle.PAPER_5) {
            ...
            app.OZ.foldedFigure.cp_worker1.setAllPointStateFalse();
            //折り上がり図でi_closestPointIdと同じ位置の点の番号を求め、cp_worker1でその番号の点を選択済みにする
            Point ps = new Point();
            ps.set(app.OZ.foldedFigure.cp_worker2.getPoint(i_closestPointId));
            for (int i = 1; i <= app.OZ.foldedFigure.cp_worker2.getPointsTotal(); i++) {
                if (ps.distance(app.OZ.foldedFigure.cp_worker2.getPoint(i)) < Epsilon.UNKNOWN_1EN7) {
                    app.OZ.foldedFigure.cp_worker1.setPointStateTrue(i);
                }
            }
        }
    }
    ...
    public void foldedFigure_operation_mouse_off_2(Point p) {//折り上がり図操作でマウスの左ボタンを離したときの作業
        ...
        if (i_nanini_near == 2) {
            ...
            if (p_m_left_on.distance(p) > Epsilon.UNKNOWN_1EN7) {
                ...
                try {
                    app.OZ.foldedFigure.folding_estimated_03();//20180216
                    app.OZ.foldedFigure_worker_drawer.calculateFromTopCountedPosition();
    ...
    public void foldedFigure_operation_mouse_on_1(Point p) {//Work when the left mouse button is pressed in the folding diagram operation Folding function
        ...
        i_closestPointId = app.OZ.wireFrame_worker_drawer1.closestPointId_with_camera(p);
        if (i_closestPointId != 0) {
            i_nanini_near = 1;
        }
        if (app.OZ.wireFrame_worker_drawer2.closestPointId_with_camera(p, app.OZ.foldedFigure.ip4) != 0) {
            if (app.OZ.wireFrame_worker_drawer1.closest_point_distance_with_camera(p) > app.OZ.wireFrame_worker_drawer2.closest_point_distance_with_camera(p, app.OZ.foldedFigure.ip4)) {
                i_closestPointId = app.OZ.wireFrame_worker_drawer2.closestPointId_with_camera(p, app.OZ.foldedFigure.ip4);
                i_nanini_near = 2;
            }
        }//Store the number of the point closest to p in i_closestPointId

        move_previous_selection_point.set(app.OZ.foldedFigure.cp_worker2.getPoint(i_closestPointId));
        ...
        if (i_nanini_near == 1) {
            ...
            if (app.OZ.foldedFigure.cp_worker1.getPointState(i_closestPointId)) {
                i_point_selection = PointSelection.WORKER_1;
            }
            if (app.OZ.foldedFigure.cp_worker2.getPointState(i_closestPointId)) {
            ...
            switch (i_point_selection) {//Find the number of the point at the same position as i_closestPointId in the fold-up diagram, and mark the point with that number as selected with cp_worker1.
                case NONE_0:
                    ...
                    app.OZ.foldedFigure.cp_worker2.changePointState(i_closestPointId);
                    break;
                case WORKER_1:
                case WORKER_2:
                    app.OZ.foldedFigure.cp_worker2.changePointState(i_closestPointId);
        ...
        if (i_nanini_near == 2) {
            ...
            if (app.OZ.foldedFigure.cp_worker1.getPointState(i_closestPointId)) {
                i_point_selection = PointSelection.WORKER_1;
                if (app.OZ.foldedFigure.cp_worker2.getSelectedPointsNum() > 0) {
                    i_point_selection = PointSelection.WORKER_2;
            ...
            if (app.canvasModel.getFoldedFigureOperationMode() == FoldedFigureOperationMode.MODE_1) {
                app.OZ.foldedFigure.display_flg_backup = app.OZ.foldedFigure.displayStyle;   //20180216  //display_flgは、折り上がり図の表示様式の指定。4なら実際に折り紙を折った場合と同じ。3なら透過図。2なら針金図。
                app.OZ.foldedFigure.displayStyle = FoldedFigure.DisplayStyle.WIRE_2;            //20180216
            }
        }

        System.out.println("cp_worker1.get_ten_sentakusuu() = " + app.OZ.foldedFigure.cp_worker1.getSelectedPointsNum());
        System.out.println("cp_worker2.get_ten_sentakusuu() = " + app.OZ.foldedFigure.cp_worker2.getSelectedPointsNum());
    }
    ...
    public void foldedFigure_operation_mouse_drag_1(Point p) {//Work when dragging while holding down the left mouse button in the fold-up diagram operation
        ...
        if (i_nanini_near == 2) {
            app.OZ.wireFrame_worker_drawer2.mDragged_selectedPoint_move_with_camera(move_previous_selection_point, p_m_left_on, p, app.OZ.foldedFigure.ip4);
