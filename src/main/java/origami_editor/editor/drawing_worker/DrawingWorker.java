...
//20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20
    ...
    public void mReleased_A_box_select(Point p0) {
        ...
        if (p19_1.distance(p0) <= 0.000001) {
            ...
            if (foldLineSet.closestLineSegmentDistance(p) < selectionDistance) {//点pに最も近い線分の番号での、その距離を返す	public double mottomo_tikai_senbun_kyori(Ten p)
                foldLineSet.closestLineSegmentSearch(p).setSelected(2);
            }
        }
    }
    ...
    public void mReleased_A_20(Point p0) {
        ...
        if (p19_1.distance(p0) <= 0.000001) {
            ...
            p.set(camera.TV2object(p0));
            if (foldLineSet.closestLineSegmentDistance(p) < selectionDistance) {//点pに最も近い線分の番号での、その距離を返す	public double mottomo_tikai_senbun_kyori(Ten p)
                foldLineSet.closestLineSegmentSearch(p).setSelected(0);
            }
        }
    }
...
//check4(0.0001);//D_nisuru0をすると、foldLineSet.D_nisuru0内でresetが実行されるため、check4のやり直しが必要。
    ...
    public void mReleased_A_04(Point p0) {
        ...
        if (foldLineSet.closestLineSegmentDistance(p) < selectionDistance) {//点pに最も近い線分の番号での、その距離を返す	public double mottomo_tikai_senbun_kyori(Ten p)
            LineSegment minris = foldLineSet.closestLineSegmentSearch(p);
...
//58 58 58 58 58 58 58 58 58 58
    ...
    public void mReleased_A_58(Point p0) {//ここの処理の終わりに fix2(0.001,0.5);　をするのは、元から折線だったものと、補助線から変換した折線との組合せで頻発するT字型不接続を修正するため
        ...
        if (p19_1.distance(p0) <= 0.000001) {//
            ...
            p.set(camera.TV2object(p0));
            if (foldLineSet.closestLineSegmentDistance(p) < selectionDistance) {//点pに最も近い線分の番号での、その距離を返す	public double mottomo_tikai_senbun_kyori(Ten p)
                LineSegment s = foldLineSet.closestLineSegmentSearch(p);
...
//66666666666666666666    mouseMode==6　;2点から等距離線分モード
    ...
    public void mPressed_A_30(Point p0) {    //マウス操作(mouseMode==4線_変換　でボタンを押したとき)時の作業
        ...
        if (foldLineSet.closestLineSegmentDistance(p) < selectionDistance) {//点pに最も近い線分の番号での、その距離を返す	public double mottomo_tikai_senbun_kyori(Ten p)
            lineSegment_ADVANCE_CREASE_TYPE_30 = foldLineSet.closestLineSegmentSearch(p);
            LineSegment s01 = new LineSegment();
            s01.set(OritaCalc.lineSegment_double(lineSegment_ADVANCE_CREASE_TYPE_30, 0.01));
            lineSegment_ADVANCE_CREASE_TYPE_30.setB(s01.getB());
        }
    }

    public void mDragged_A_30(Point p0) {//マウス操作(mouseMode==4線_変換　でドラッグしたとき)を行う関数
        if (lineSegment_ADVANCE_CREASE_TYPE_30 != null) {

            LineSegment s01 = new LineSegment();
            s01.set(OritaCalc.lineSegment_double(lineSegment_ADVANCE_CREASE_TYPE_30, 100.0));
            lineSegment_ADVANCE_CREASE_TYPE_30.setB(s01.getB());
            lineSegment_ADVANCE_CREASE_TYPE_30 = null;
        }
    ...
    public void mReleased_A_30(Point p0) {
        ...
        if (lineSegment_ADVANCE_CREASE_TYPE_30 != null) {
            ...
            LineColor ic_temp = lineSegment_ADVANCE_CREASE_TYPE_30.getColor();
            int is_temp = lineSegment_ADVANCE_CREASE_TYPE_30.getSelected();

            if ((ic_temp == LineColor.BLACK_0) && (is_temp == 0)) {
                lineSegment_ADVANCE_CREASE_TYPE_30.setSelected(2);
            } else if ((ic_temp == LineColor.BLACK_0) && (is_temp == 2)) {
                lineSegment_ADVANCE_CREASE_TYPE_30.setColor(LineColor.RED_1);
                lineSegment_ADVANCE_CREASE_TYPE_30.setSelected(0);
            } else if ((ic_temp == LineColor.RED_1) && (is_temp == 0)) {
                lineSegment_ADVANCE_CREASE_TYPE_30.setColor(LineColor.BLUE_2);
            } else if ((ic_temp == LineColor.BLUE_2) && (is_temp == 0)) {
                lineSegment_ADVANCE_CREASE_TYPE_30.setColor(LineColor.BLACK_0);
            }
        ...
        if (i_drawing_stage == 1) {
            ...
            if (p.distance(closest_point) <= selectionDistance) {
                if (line_step[1].getLength() > 0.00000001) {
                    ...
                    for (int i = 1; i <= imax; i++) {
                        LineSegment s = foldLineSet.get(i);
                        LineSegment.Intersection i_lineSegment_intersection_decision = OritaCalc.line_intersect_decide_sweet(s, line_step[1], 0.01, 0.01);
                        int i_jikkou = 0;
                        ...
                        if (i_jikkou == 1) {
                            ...
                            if (OritaCalc.distance_lineSegment(t_moto, line_step[1]) < OritaCalc.distance_lineSegment(s.getB(), line_step[1])) {
                                t_moto.set(s.getB());
                            }
                            ...
                            LineSegment add_sen = new LineSegment(OritaCalc.findIntersection(s, line_step[1]), t_taisyou);
    ...
    public void mReleased_A_36(Point p0) {
        ...
        if (i_drawing_stage == 1) {
            ...
            if (line_step[1].getLength() > 0.00000001) {
                for (int i = 1; i <= foldLineSet.getTotal(); i++) {
                    LineSegment s = foldLineSet.get(i);
                    LineSegment.Intersection i_senbun_kousa_hantei = OritaCalc.line_intersect_decide(s, line_step[1], 0.0001, 0.0001);
                    int i_jikkou = 0;
                    ...
                    if (i_jikkou == 1) {
                        WeightedValue<LineSegment> i_d = new WeightedValue<>(s, OritaCalc.distance(line_step[1].getB(), OritaCalc.findIntersection(s, line_step[1])));
                        nbox.container_i_smallest_first(i_d);
                    }
        ...
        if (i_O_F_C) {
            ...
            for (int i_s_step = 1; i_s_step <= i_drawing_stage; i_s_step++) {
                ...
                for (int i = 1; i <= foldLineSet.getTotal(); i++) {
                    ...
                    LineSegment.Intersection i_senbun_kousa_hantei = OritaCalc.line_intersect_decide(s, line_step[i_s_step], 0.0001, 0.0001);
                    int i_jikkou = 0;
                    ...
                    if (i_jikkou == 1) {
                        WeightedValue<LineSegment> i_d = new WeightedValue<>(s, OritaCalc.distance(line_step[i_s_step].getA(), OritaCalc.findIntersection(s, line_step[i_s_step])));
                        nbox.container_i_smallest_first(i_d);
                    }
                }
...
//14 14 14 14 14 14 14 14 14    mouseMode==14　;V追加モード
    ...
    public void mPressed_A_14(Point p0) {
        //Ten p =new Ten();
        p.set(camera.TV2object(p0));
        LineSegment mtsLineSegment = foldLineSet.closestLineSegmentSearch(p);
        ...
        if (OritaCalc.distance_lineSegment(p, mts) < selectionDistance) {
            ...
            if (OritaCalc.isInside(mts.getA(), pk, mts.getB()) == 2) {
                //線分の分割-----------------------------------------
                foldLineSet.lineSegment_bunkatu(mtsLineSegment, pk);  //i番目の線分(端点aとb)を点pで分割する。i番目の線分abをapに変え、線分pbを加える。
                record();
            }
    ...
    public void mReleased_A_23(Point p0) {//ここの処理の終わりに fix2(0.001,0.5);　をするのは、元から折線だったものと、補助線から変換した折線との組合せで頻発するT字型不接続を修正するため
        ...
        if (p19_1.distance(p0) <= 0.000001) {//現状では赤を赤に変えたときもUNDO用に記録されてしまう20161218
            ...
            if (foldLineSet.closestLineSegmentDistance(p) < selectionDistance) {//点pに最も近い線分の番号での、その距離を返す	public double closestLineSegmentDistance(Ten p)
                foldLineSet.closestLineSegmentSearch(p).setColor(LineColor.RED_1);
                fix2(0.001, 0.5);
                record();
            }
        }
...
//24 24 24 24 24
    ...
    public void mReleased_A_24(Point p0) {//ここの処理の終わりに fix2(0.001,0.5);　をするのは、元から折線だったものと、補助線から変換した折線との組合せで頻発するT字型不接続を修正するため
        ...
        if (p19_1.distance(p0) <= 0.000001) {
            p.set(camera.TV2object(p0));
            if (foldLineSet.closestLineSegmentDistance(p) < selectionDistance) {//点pに最も近い線分の番号での、その距離を返す	public double mottomo_tikai_senbun_kyori(Ten p)
                foldLineSet.closestLineSegmentSearch(p).setColor(LineColor.BLUE_2);
                fix2(0.001, 0.5);
                record();
            }
        }
...
//25 25 25 25 25
    ...
    public void mReleased_A_25(Point p0) {//ここの処理の終わりに fix2(0.001,0.5);　をするのは、元から折線だったものと、補助線から変換した折線との組合せで頻発するT字型不接続を修正するため
        ...
        if (p19_1.distance(p0) <= 0.000001) {
            ...
            p.set(camera.TV2object(p0));
            if (foldLineSet.closestLineSegmentDistance(p) < selectionDistance) {//点pに最も近い線分の番号での、その距離を返す	public double mottomo_tikai_senbun_kyori(Ten p)
                foldLineSet.closestLineSegmentSearch(p).setColor(LineColor.BLACK_0);
                fix2(0.001, 0.5);
                record();
            }
        }
