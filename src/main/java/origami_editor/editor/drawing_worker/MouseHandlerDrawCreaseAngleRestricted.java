...
public class MouseHandlerDrawCreaseAngleRestricted extends BaseMouseHandler{
    ...
    public MouseHandlerDrawCreaseAngleRestricted(DrawingWorker d) {
        super(d);
    }
    ...
    public void mousePressed(Point p0) {
        ...
        if (d.i_drawing_stage == 0) {    //第１段階として、線分を選択
        ...
        if (d.i_drawing_stage == i_jyunnbi_step_suu + (honsuu) + (honsuu)) {//19     //動作の準備としてソフトが返答するステップ数が終わった状態で実行
            ...
            d.closest_lineSegment.set(d.get_moyori_step_lineSegment(p, 2, 1 + (honsuu)));
            if (OritaCalc.distance_lineSegment(p, d.closest_lineSegment) < d.selectionDistance) {
                d.i_drawing_stage++;
                i_tikai_s_step_suu++;
            ...
            //line_step[2から10]までとs_step[11から19]まで
            d.closest_lineSegment.set(d.get_moyori_step_lineSegment(p, 1 + (honsuu) + 1, 1 + (honsuu) + (honsuu)));
            if (OritaCalc.distance_lineSegment(p, d.closest_lineSegment) < d.selectionDistance) {
                d.i_drawing_stage++;
                i_tikai_s_step_suu++;
            ...
            if (i_tikai_s_step_suu == 2) { //この段階でs_stepが[21]までうまってたら、line_step[20]とs_step[21]は共に加える折線なので、ここで処理を終えてしまう。
                ...
                kousa_point.set(OritaCalc.findIntersection(d.line_step[1 + (honsuu) + (honsuu) + 1], d.line_step[1 + (honsuu) + (honsuu) + 1 + 1]));

                LineSegment add_sen = new LineSegment(kousa_point, d.line_step[1 + (honsuu) + (honsuu) + 1].getA());
                add_sen.setColor(d.lineColor);
                if (add_sen.getLength() > 0.00000001) {
                    d.addLineSegment(add_sen);
                }

                LineSegment add_sen2 = new LineSegment(kousa_point, d.line_step[1 + (honsuu) + (honsuu) + 1 + 1].getA());
